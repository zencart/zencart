 ...
 */
class ListStandard extends AbstractFormatter
{
    /**
     * @var
     */
    protected $displayValues;
    protected $displayEntities;
    protected $prefix;
    protected $groupId;
    ...
    protected function processItems($items)
    {
        ...
        $this->prefix = $outputLayout ['formatter']['params']['definePrefix'];
        $this->imgWidth = $outputLayout ['formatter']['params']['imageListingWidth'];
        $this->imgHeight = $outputLayout ['formatter']['params']['imageListingHeight'];
        $this->groupId = zen_get_configuration_key_value($this->prefix . 'LIST_GROUP_ID');
        $this->displayEntities = array('LIST_IMAGE' => array(),
                                       'LIST_NAME' => array(),
        ...
        $columnOneSortOrders = $db->execute($columnSql['one']);
        $columnTwoSortOrders = $db->execute($columnSql['two']);
        $this->countQuantityBoxItems = 0;
        foreach ($items as $item) {
            $item = $this->setExtraParameters($item);
            $this->initDisplayEntities($item);
     ...
     * @param $item
     */
    protected function initDisplayEntities($item)
    {
        foreach ($this->displayEntities as $key => $value) {
    ...
    protected function getColumnSql()
    {
        ...
        $sql['one'] = "SELECT configuration_key, configuration_value
                FROM " . TABLE_CONFIGURATION . " WHERE configuration_group_id='" . $this->groupId . "'
    ...
    protected function processColumnEntries($queryResult, $columnName, $item)
    {
        ...
        $item['displayProductsDescription'] = $this->displayValues['LIST_DESCRIPTION'];
        return $item;
    }
    ...
    protected function processorListImage($item, $key)
    {
        $result = '<a href="' . zen_href_link($item['product_info_page'], 'cPath=' .
            ...
            zen_image(DIR_WS_IMAGES . $item ['products_image'], $item ['products_name'], $this->imgWidth, $this->imgHeight) .
    ...
    /**
     * @param string $link
     * @param boolean $allowAddCart
     * @param boolean $hasAttributes
     * @param boolean $multiAddCart
     * @param $item
     * @return string
     */
