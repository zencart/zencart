 ...
 */
class TabularProduct extends AbstractFormatter
{
    /**
     * @return array
     */
    public function format()
    {
        ...
        $listBoxContents = $this->buildColumnOptions($items);
     ...
     * @return array
     */
    protected function buildColumnOptions($items)
    {
        $listBoxContents = array();
        $headers = array();
        if (count($items) == 0) {
            $this->diContainer->get('listingBox')->setTemplateVariable('caption', TEXT_NO_PRODUCTS);
            return array();
        }
        $headersLayoutMap = $this->getHeadersLayoutMap();
        for ($col = 0, $n = sizeof($this->columnList); $col < $n; $col++) {
            $result = $headersLayoutMap[$this->columnList [$col]]();
            $headers [] = array(
                'title' => $result['title'],
                'col_params' => $result['col_params']
            );
        }
        $this->diContainer->get('listingBox')->setTemplateVariable('headers', $headers);
        $this->diContainer->get('listingBox')->setTemplateVariable('maxColSpan', count($headers));
        ...
        foreach ($items as $item) {
            ...
            for ($col = 0, $n = sizeof($this->columnList); $col < $n; $col++) {
                $result = $rowLayoutMap[$this->columnList [$col]]();
                $row [] = $result;
            }
            $listBoxContents [] = $row;
        }
    ...
    protected function getProductLink($item)
    {
        ...
        $cPathGenOpt = ($mfId > 0 and $filterId > 0) ? $filterId : ($cPath != '') ? $cPath : $item ['master_categories_id'];
