...
class QueryBuilder extends \base
{
    ...
    public function init($diContainer)
    {
        ...
        $productQuery = $diContainer->get('listingBox')->getProductQuery();
    ...
    public function processQuery()
    {
        ...
        if (issetorArray($productQuery, 'isPaginated', false) && !isset($this->query['countSql'])) {
    ...
    protected function processBindVars()
    {
        ...
        foreach ($this->parts ['bindVars'] as $bindVars) {
            $this->query['mainSql'] = $this->diContainer->get('dbConn')->bindVars($this->query['mainSql'], $bindVars [0], $bindVars [1], $bindVars [2]);
            if (isset($this->query['countSql'])) {
                $this->query['countSql'] = $this->diContainer->get('dbConn')->bindVars($this->query['countSql'], $bindVars [0], $bindVars [1], $bindVars [2]);
            }
        }
    ...
    public function executeQuery()
    {
        ...
        $productQuery = $this->diContainer->get('listingBox')->getProductQuery();
        $isPaginated = issetorArray($productQuery, 'isPaginated', false);
        $queryLimit = issetorArray($productQuery, 'queryLimit', null);

        $resultItems = array();
        $this->processBindVars();
        if ($isPaginated) {
            $this->diContainer->get('paginator')->setData(array('dbConn' => $this->diContainer->get('dbConn'),
                                                                'sqlQueries' => array('main' => $this->query['mainSql'],
                                                                                      'count' => $this->query['countSql'])));
            $this->diContainer->get('paginator')->init();
            $this->diContainer->get('paginator')->process();
            $resultItems = $this->diContainer->get('paginator')->getItems();
        } else {
            $results = $this->diContainer->get('dbConn')->execute($this->query['mainSql'], $queryLimit);
            $this->notify('NOTIFY_QUERY_BUILDER_EXECUTEQUERY_RESULT', null, $results);
            foreach ($results as $result) {
                $resultItems [] = $result;
            }
        }
        foreach ($resultItems as $resultItem) {
            $resultItem = $this->diContainer->get('derivedItemManager')->manageDerivedItems($this->parts ['derivedItems'], $resultItem);
