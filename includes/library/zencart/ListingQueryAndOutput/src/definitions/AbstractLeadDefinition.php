...
{
    /**
     * AbstractLeadDefinition constructor.
    ...
    {
        foreach ($this->pageDefinition ['extraRowActions'] as $extraRowActions) {
    ...
    protected function processItemCallables($row, $item)
    {
        ...
        foreach ($this->pageDefinition ['fields'] as $field => $options) {
    ...
    {
        if (isset($fieldFormatter['context']) && in_array($this->pageDefinition ['action'], $fieldFormatter['context'])) {
            return true;
        }
        if (!isset($fieldFormatter['context']) && $this->pageDefinition ['action'] == 'list') {
            return true;
        }
    }
