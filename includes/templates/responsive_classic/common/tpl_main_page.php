...
if ($flag_disable_right || $tplSetting->COLUMN_RIGHT_STATUS === '0') {
  $box_width_right = preg_replace('/[^0-9]/', '', '0');
  $box_width_right_new = '';
} else {
  $box_width_right = $tplSetting->COLUMN_WIDTH_RIGHT;
  $box_width_right = preg_replace('/[^0-9]/', '', $box_width_right);
  $box_width_right_new = 'col' . $box_width_right;
}

if ($flag_disable_left || $tplSetting->COLUMN_LEFT_STATUS === '0') {
  $box_width_left = preg_replace('/[^0-9]/', '', '0');
  $box_width_left_new = '';
} else {
  $box_width_left = $tplSetting->COLUMN_WIDTH_LEFT;
...
<?php
if ((int)$tplSetting->COLUMN_LEFT_STATUS === 0 || (zen_config('CUSTOMERS_APPROVAL') === '1' && !zen_is_logged_in()) || ((int)zen_config('CUSTOMERS_APPROVAL_AUTHORIZATION') === 1 && zen_config('CUSTOMERS_AUTHORIZATION_COLUMN_LEFT_OFF') === 'true' && ($_SESSION['customers_authorization'] != 0 || !zen_is_logged_in()))) {
...
<!-- bof  breadcrumb -->
<?php if (!$breadcrumb->isEmpty() && ($tplSetting->DEFINE_BREADCRUMB_STATUS === '1' || ($tplSetting->DEFINE_BREADCRUMB_STATUS === '2' && !$this_is_home_page))) { ?>
