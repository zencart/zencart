...
$limit = (trim($tplSetting->MAX_DISPLAY_BESTSELLERS) === '') ? '' : (' LIMIT ' . (int)$tplSetting->MAX_DISPLAY_BESTSELLERS);
$best_sellers_query .= $limit;
$best_sellers = $db->Execute($best_sellers_query);
if ($best_sellers->RecordCount() >= $tplSetting->MIN_DISPLAY_BESTSELLERS) {
    $rows = 0;
    $bestsellers_list = [];
    foreach ($best_sellers as $bestseller) {
        ...
        $bestsellers_list[$rows] = [
            ...
            'image' => zen_image(DIR_WS_IMAGES . $bestseller['products_image'], $bestseller['products_name'], $tplSetting->SMALL_IMAGE_WIDTH, $tplSetting->SMALL_IMAGE_HEIGHT),
