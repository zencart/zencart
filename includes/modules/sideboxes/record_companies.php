...
// Display a list
    ...
    foreach ($record_company as $next_company) {
        ...
        if (mb_strlen($record_company_name) > (int)$tplSetting->MAX_DISPLAY_RECORD_COMPANY_NAME_LEN) {
            $record_company_name = mb_substr($record_company_name, 0, (int)$tplSetting->MAX_DISPLAY_RECORD_COMPANY_NAME_LEN) . '..';
        }
        $record_company_array[] = [
