 ...
 * @version $Id: lat9 2023 Apr 20 Modified in v2.0.0-alpha1 $
 */
...
foreach ($keywords_metatags as $next_keyword) {
    $keywords_string_metatags .= zen_clean_html($next_keyword['categories_name']) . METATAGS_DIVIDER;
}
...
if ($metatag_page_name !== 'index') {
    if (defined('META_TAG_TITLE_' . strtoupper($metatag_page_name))) {
        zen_define_default('META_TAG_TITLE', constant('META_TAG_TITLE_' . strtoupper($metatag_page_name)));
    }
    if (defined('META_TAG_DESCRIPTION_' . strtoupper($metatag_page_name))) {
        zen_define_default('META_TAG_DESCRIPTION', constant('META_TAG_DESCRIPTION_' . strtoupper($metatag_page_name)));
    }
    if (defined('META_TAG_KEYWORDS_' . strtoupper($metatag_page_name))) {
        zen_define_default('META_TAG_KEYWORDS', constant('META_TAG_KEYWORDS_' . strtoupper($metatag_page_name)));
    }
}
...
switch ($metatag_page_name) {
    ...
    case 'address_book':
        zen_define_default('META_TAG_TITLE', HEADING_TITLE . PRIMARY_SECTION . TITLE . TAGLINE);
        zen_define_default('META_TAG_DESCRIPTION', TITLE . PRIMARY_SECTION . NAVBAR_TITLE_1 . SECONDARY_SECTION . KEYWORDS);
        zen_define_default('META_TAG_KEYWORDS', KEYWORDS . METATAGS_DIVIDER . NAVBAR_TITLE_1);
        break;

    case 'address_book_process':
        if (is_numeric($_GET['edit'] ?? 'abc')) {
            zen_define_default('META_TAG_TITLE', NAVBAR_TITLE_MODIFY_ENTRY . PRIMARY_SECTION . TITLE . TAGLINE);
        } elseif (is_numeric($_GET['delete'] ?? 'abc')) {
            zen_define_default('META_TAG_TITLE', NAVBAR_TITLE_DELETE_ENTRY . PRIMARY_SECTION . TITLE . TAGLINE);
        } else {
            zen_define_default('META_TAG_TITLE', NAVBAR_TITLE_ADD_ENTRY . PRIMARY_SECTION . TITLE . TAGLINE);
        }
        zen_define_default('META_TAG_DESCRIPTION', TITLE . PRIMARY_SECTION . NAVBAR_TITLE_ADD_ENTRY . SECONDARY_SECTION . KEYWORDS);
        zen_define_default('META_TAG_KEYWORDS', KEYWORDS . METATAGS_DIVIDER . NAVBAR_TITLE_ADD_ENTRY);
        break;

    case 'search_result':
    case 'advanced_search_result':
        zen_define_default('META_TAG_TITLE', NAVBAR_TITLE_2 . ' -> ' . zen_output_string_protected($keywords) . ' ' . PRIMARY_SECTION . TITLE . TAGLINE);
        zen_define_default('META_TAG_DESCRIPTION', '');
        zen_define_default('META_TAG_KEYWORDS', '');
        break;

    case 'password_forgotten':
        zen_define_default('META_TAG_TITLE', NAVBAR_TITLE_2 . PRIMARY_SECTION . TITLE . TAGLINE);
        zen_define_default('META_TAG_DESCRIPTION', TITLE . PRIMARY_SECTION . NAVBAR_TITLE_2 . SECONDARY_SECTION . KEYWORDS);
        zen_define_default('META_TAG_KEYWORDS', KEYWORDS . METATAGS_DIVIDER . NAVBAR_TITLE_2);
        break;
    ...
    case 'create_account_success':
        zen_define_default('META_TAG_TITLE', HEADING_TITLE . PRIMARY_SECTION . TITLE . TAGLINE);
        zen_define_default('META_TAG_DESCRIPTION', TITLE . PRIMARY_SECTION . HEADING_TITLE . SECONDARY_SECTION . KEYWORDS);
        zen_define_default('META_TAG_KEYWORDS', KEYWORDS . METATAGS_DIVIDER . HEADING_TITLE);
        break;

    case ($this_is_home_page):
        zen_define_default('META_TAG_TITLE', (defined('HOME_PAGE_TITLE') && HOME_PAGE_TITLE !== '' ? HOME_PAGE_TITLE : (defined('NAVBAR_TITLE') ? NAVBAR_TITLE . PRIMARY_SECTION : '') . TITLE . TAGLINE));
        zen_define_default('META_TAG_DESCRIPTION', (defined('HOME_PAGE_META_DESCRIPTION') && HOME_PAGE_META_DESCRIPTION !== '') ? HOME_PAGE_META_DESCRIPTION : TITLE . PRIMARY_SECTION . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) . SECONDARY_SECTION . KEYWORDS);
        zen_define_default('META_TAG_KEYWORDS', (defined('HOME_PAGE_META_KEYWORDS') && HOME_PAGE_META_KEYWORDS !== '') ? HOME_PAGE_META_KEYWORDS : KEYWORDS . METATAGS_DIVIDER . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) );
        break;
    ...
    case 'index':
        ...
        $sql =
            "SELECT *
               FROM " . TABLE_METATAGS_CATEGORIES_DESCRIPTION . " mcd
              WHERE mcd.categories_id = " . (int)$current_category_id . "
                AND mcd.language_id = " . (int)$_SESSION['languages_id'];
    ...
    case 'popup_image':
        ...
        zen_define_default('META_TAG_TITLE', $meta_products_name . PRIMARY_SECTION . TITLE . TAGLINE);
        zen_define_default('META_TAG_DESCRIPTION', TITLE . PRIMARY_SECTION . $meta_products_name . SECONDARY_SECTION . KEYWORDS);
        zen_define_default('META_TAG_KEYWORDS', KEYWORDS . METATAGS_DIVIDER . $meta_products_name);
        break;
    ...
    case 'popup_search_help':
        zen_define_default('META_TAG_TITLE', HEADING_SEARCH_HELP . PRIMARY_SECTION . TITLE . TAGLINE);
        zen_define_default('META_TAG_DESCRIPTION', TITLE . PRIMARY_SECTION . HEADING_SEARCH_HELP . SECONDARY_SECTION . KEYWORDS);
        zen_define_default('META_TAG_KEYWORDS', KEYWORDS . METATAGS_DIVIDER . HEADING_SEARCH_HELP);
        break;
    ...
    //  case 'product_info':
    case (strstr($_GET['main_page'], 'product_') || strstr($_GET['main_page'], 'document_')):
    ...
    case 'page':
        ...
        $chapter_id = (int)($_GET['chapter'] ?? 0);
        if (defined('META_TAG_TITLE_EZPAGE_' . $ezpage_id)) {
            zen_define_default('META_TAG_TITLE', constant('META_TAG_TITLE_EZPAGE_'.$ezpage_id));
        }
        if (defined('META_TAG_DESCRIPTION_EZPAGE_' . $ezpage_id)) {
            zen_define_default('META_TAG_DESCRIPTION', constant('META_TAG_DESCRIPTION_EZPAGE_'.$ezpage_id));
        }
        if (defined('META_TAG_KEYWORDS_EZPAGE_' . $ezpage_id)) {
            zen_define_default('META_TAG_KEYWORDS', constant('META_TAG_KEYWORDS_EZPAGE_'.$ezpage_id));
        }
