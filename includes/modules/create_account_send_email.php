...
if (zen_config('NEW_SIGNUP_DISCOUNT_COUPON', '') !== '' && zen_config('NEW_SIGNUP_DISCOUNT_COUPON') !== '0') {
    $coupon_id = (int)zen_config('NEW_SIGNUP_DISCOUNT_COUPON');
    $coupon = $db->Execute(
...
if ((float)zen_config('NEW_SIGNUP_GIFT_VOUCHER_AMOUNT') > 0) {
    ...
    $insert_query = $db->Execute(
        "INSERT INTO " . TABLE_COUPONS . "
         ...
         VALUES
            ('" . $coupon_code . "', 'G', '" . (float)zen_config('NEW_SIGNUP_GIFT_VOUCHER_AMOUNT') . "', now())"
    );
    ...
    $email_text .=
        "\n\n" . sprintf(EMAIL_GV_INCENTIVE_HEADER, $currencies->format(zen_config('NEW_SIGNUP_GIFT_VOUCHER_AMOUNT'))) .
...
if (IS_ADMIN_FLAG === false && zen_config('SEND_EXTRA_CREATE_ACCOUNT_EMAILS_TO_STATUS') === '1' && zen_config('SEND_EXTRA_CREATE_ACCOUNT_EMAILS_TO', '') !== '' && isset($_SESSION['customer_id'])) {
    ...
    if (trim(SEND_EXTRA_CREATE_ACCOUNT_EMAILS_TO_SUBJECT) !== 'n/a') {
        zen_mail('', zen_config('SEND_EXTRA_CREATE_ACCOUNT_EMAILS_TO'), SEND_EXTRA_CREATE_ACCOUNT_EMAILS_TO_SUBJECT . ' ' . EMAIL_SUBJECT, $email_text . $extra_info['TEXT'], zen_config('STORE_NAME'), zen_config('EMAIL_FROM'), $html_msg, 'welcome_extra');
    }
