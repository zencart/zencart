...
// Slam prevention:
    ...
    if ($check_customer->RecordCount() > 0) {
        ...
        // send the email
        zen_mail($check_customer->fields['customers_firstname'] . ' ' . $check_customer->fields['customers_lastname'],
            $email_address, EMAIL_PASSWORD_REMINDER_SUBJECT, sprintf(EMAIL_PASSWORD_REMINDER_BODY, $new_password),
            STORE_NAME, EMAIL_FROM, $html_msg, 'password_forgotten');

        // handle 3rd-party integrations
        $zco_notifier->notify('NOTIFY_PASSWORD_FORGOTTEN_CHANGED', $email_address,
            $check_customer->fields['customers_id'], $new_password);
        if (PADSS_ADMIN_SESSION_TIMEOUT_ENFORCED) {
            $messageStack->add_session('login', SUCCESS_PASSWORD_SENT, 'success');
        } else {
            $messageStack->add_session('login', SUCCESS_PASSWORD_SENT_CONFIRM, 'success');
        }
        ...
        if (PADSS_ADMIN_SESSION_TIMEOUT_ENFORCED) {
            ...
            $messageStack->add('password_forgotten', EMAIL_ADDRESS_NOT_FOUND);
        }
    }
}
