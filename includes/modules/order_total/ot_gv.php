  ...
  function update_credit_account($i) {
    ...
    if (preg_match('/^GIFT/', addslashes($order->products[$i]['model']))) {
      ...
      $gv_original_price = zen_products_lookup((int)$order->products[$i]['id'], 'products_price');
       // if prices differ assume Special and get Special Price (coming in v156)
       // Do not use this on GVs Priced by Attribute
      if (defined('MODULE_ORDER_TOTAL_GV_SPECIAL') && MODULE_ORDER_TOTAL_GV_SPECIAL == 'true' && ($gv_original_price != 0 && $gv_original_price != $order->products[$i]['final_price'] && !zen_get_products_price_is_priced_by_attributes((int)$order->products[$i]['id']))) {
