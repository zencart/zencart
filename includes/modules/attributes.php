        ...
        if ($products_options_type == PRODUCTS_OPTIONS_TYPE_TEXT) {
            if (!empty($_POST['id']) && is_array($_POST['id'])) {
                ...
                if ($products_options_names->fields['products_options_rows'] > 1) {
                    ...
                    $tmp_html = '<input type="text" name="id[' . TEXT_PREFIX . $products_options_id . ']" size="' . $products_options_names->fields['products_options_size'] . '" maxlength="' . $products_options_names->fields['products_options_length'] . '" value="' . htmlspecialchars($tmp_value, ENT_COMPAT, CHARSET) . '" id="' . $inputFieldId . '"'  . $data_properties . $field_disabled . '>  ';
                }
                $tmp_html .= $products_options_details;
