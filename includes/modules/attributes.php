    ...
    */
    ...
    foreach ($products_options as $next_option) {
        ...
        if (
            ...
            || ((zen_config('CUSTOMERS_APPROVAL_AUTHORIZATION') === '1' || zen_config('CUSTOMERS_APPROVAL_AUTHORIZATION') === '2') && $_SESSION['customers_authorization'] == '')
                        ...
                        $tmp_attributes_image .=
                            '<div class="attribImg">' .
                                ...
                                '<label class="attribsRadioButton three" for="' . $inputFieldId . '">' .
                                    ...
                                    (zen_config('PRODUCT_IMAGES_ATTRIBUTES_NAMES') === '1' ? '<br>' . $next_option['products_options_values_name'] : '') . $products_options_details_noname .
                                '</label>' .
                            '</div>' . "\n";
                    } else {
                        $tmp_attributes_image .=
                        ...
                        $tmp_attributes_image .=
                            '<div class="attribImg">' .
                                '<label class="attribsRadioButton four" for="' . $inputFieldId . '">' .
                                    zen_image(DIR_WS_IMAGES . $next_option['attributes_image']) . (zen_config('PRODUCT_IMAGES_ATTRIBUTES_NAMES') === '1' ? '<br>' . $next_option['products_options_values_name'] : '') .
        ...
        if ($products_options_type === zen_config('PRODUCTS_OPTIONS_TYPE_TEXT')) {
            $option_form_name = 'id[' . zen_config('TEXT_PREFIX') . $products_options_id . ']';
            if (!empty($_POST['id']) && is_array($_POST['id'])) {
                foreach ($_POST['id'] as $key => $value) {
                    if (preg_replace('/txt_/', '', $key) == $products_options_id) {
                        ...
                        if ($next_option_name['products_options_rows'] > 1) {
                            $tmp_html =
                                '  <input disabled="disabled" type="text" name="remaining' . zen_config('TEXT_PREFIX') . $products_options_id . '" size="3" maxlength="3" value="' . $next_option_name['products_options_length'] . '"> ' .
                                TEXT_MAXIMUM_CHARACTERS_ALLOWED .
                                '<br>';
                            $tmp_html .= '<textarea class="attribsTextarea" name="' . $option_form_name . '" rows="' . $next_option_name['products_options_rows'] . '" cols="' . $next_option_name['products_options_size'] . '" onkeydown="characterCount(this.form[\'' . $option_form_name . '\'],this.form.remaining' . zen_config('TEXT_PREFIX') . $products_options_id . ',' . $next_option_name['products_options_length'] . ');" onKeyUp="characterCount(this.form[\'' . $option_form_name . '\'],this.form.remaining' . zen_config('TEXT_PREFIX') . $products_options_id . ',' . $next_option_name['products_options_length'] . ');" id="' . $inputFieldId . '">' . stripslashes($value) . '</textarea>' . "\n";
                        } else {
                ...
                // use text area or input box based on setting of products_options_rows in the products_options table
                if ($next_option_name['products_options_rows'] > 1) {
                    $tmp_html = '  <input disabled="disabled" type="text" name="remaining' . zen_config('TEXT_PREFIX') . $products_options_id . '" size="3" maxlength="3" value="' . $next_option_name['products_options_length'] . '"> ' . TEXT_MAXIMUM_CHARACTERS_ALLOWED . '<br>';
                ...
                if (zen_config('ATTRIBUTES_ENABLED_TEXT_PRICES') === 'true') { // test ATTRIBUTES_ENABLED_TEXT_PRICES
                    $tmp_word_cnt_string = '';
        ...
        if ($products_options_type === zen_config('PRODUCTS_OPTIONS_TYPE_FILE')) {
            ...
            if (zen_run_normal() && zen_check_show_prices()) {
                ...
                $tmp_html = '<input type="file" name="id[' . zen_config('TEXT_PREFIX') . $products_options_id . ']"  id="' . $inputFieldId . '" ' . $data_properties . '><br>' . $file_attribute_value . "\n" .
