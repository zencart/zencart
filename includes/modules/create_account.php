...
$newsletter = !(zen_config('ACCOUNT_NEWSLETTER_STATUS') === '1' || zen_config('ACCOUNT_NEWSLETTER_STATUS') === '0');
$extra_welcome_text = '';
...
if (isset($_POST['action']) && ($_POST['action'] === 'process') && !isset($login_page)) {
    ...
    $newsletter = 0;
    if ((zen_config('ACCOUNT_NEWSLETTER_STATUS') === '1' || zen_config('ACCOUNT_NEWSLETTER_STATUS') === '2') && isset($_POST['newsletter'])) {
    ...
    if (zen_config('ACCOUNT_DOB') === 'true') {
        if (zen_config('ENTRY_DOB_MIN_LENGTH') > 0 or !empty($_POST['dob'])) {
