...
// TABLES: categories
  function zen_get_parent_categories(&$categories, $categories_id) {
    ...
    while (!$parent_categories->EOF) {
      if ($parent_categories->fields['parent_id'] == TOPMOST_CATEGORY_PARENT_ID) return true;
...
//// bof: manage master_categories_id vs cPath
  function zen_generate_category_path($id, $from = 'category', $categories_array = '', $index = 0) {
    ...
    if ($from == 'product') {
      ...
      while (!$categories->EOF) {
        if ($categories->fields['categories_id'] == TOPMOST_CATEGORY_PARENT_ID) {
          $categories_array[$index][] = array('id' => TOPMOST_CATEGORY_PARENT_ID, 'text' => TEXT_TOP);
        } else {
