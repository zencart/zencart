...
function plugin_version_check_for_updates($plugin_file_id = 0, $version_string_to_compare = '')
{
  ...
  $searchid = 0;
  $url = 'http://www.zen-cart.com/downloads.php?do=versioncheck';
  $url .= '&id='.(int)$plugin_file_id;
  $data = json_decode(file_get_contents($url), true);
  // compare values
  if (strcmp($data[$searchid]['latest_plugin_version'], $version_string_to_compare) > 0) return $data[$searchid];
  return FALSE;
}
