<?php
/**
 * piece_style_filter.php  for index filters
 *
 * index filter for the piece product type
 * show the products of a specified piece_style
 *
 ...
 * @version $Id: piece_style_filter.php 15628 2010-03-07 01:21:55Z drbyte $
 */
...
if (!isset($select_column_list)) $select_column_list = "";
  ...
  if (isset($_GET['piece_style_id']))
  {
    if (isset($_GET['filter_id']) && zen_not_null($_GET['filter_id']))
    {
      ...
      $listing_sql = "select " . $select_column_list . " p.products_id, p.products_type, p.master_categories_id, p.products_price, p.products_tax_class_id, pd.products_description, if(s.status = 1, s.specials_new_products_price, NULL) AS specials_new_products_price, IF(s.status = 1, s.specials_new_products_price, p.products_price) as final_price, p.products_sort_order, p.product_is_call, p.product_is_always_free_shipping, p.products_qty_box_status
        ...
        TABLE_PIECE_STYLE . " m
        where  m.piece_style_id = '" . (int)$_GET['piece_style_id'] . "'
    ...
    if (isset($_GET['filter_id']) && zen_not_null($_GET['filter_id']))
    {
      ...
      $listing_sql = "select " . $select_column_list . " p.products_id, p.products_type, p.master_categories_id, m.piece_style_id, p.products_price, p.products_tax_class_id, pd.products_description, IF(s.status = 1, s.specials_new_products_price, NULL) as specials_new_products_price, IF(s.status = 1, s.specials_new_products_price, p.products_price) as final_price, p.products_sort_order, p.product_is_call, p.product_is_always_free_shipping, p.products_qty_box_status
        ...
        where p.products_status = 1
          and pme.piece_style_id = m.piece_style_id
          and m.piece_style_id = '" . (int)$_GET['filter_id'] . "'
      ...
      if ($current_categories_id) {
        ...
        TABLE_PRODUCTS_TO_CATEGORIES . " p2c, " .
        TABLE_PIECE_STYLE . " m
        where  m.piece_style_id = pme.piece_style_id
          and p.products_id = pme.products_id
          and p.products_status = 1
          and pd.products_id = pme.products_id
  ...
  $listing_sql = str_replace('m.manufacturers_name', 'm.piece_style_name as manufacturers_name', $listing_sql);
  ...
  if (isset($column_list)) {
    if ( (!isset($_GET['sort'])) || (isset($_GET['sort']) && !preg_match('/[1-8][ad]/', $_GET['sort'])) || (substr($_GET['sort'], 0, 1) > sizeof($column_list)) )
    {
      ...
      switch ($column_list[$sort_col-1])
      {
        ...
        $listing_sql .= " order by m.piece_style_name " . ($sort_order == 'd' ? 'desc' : '') . ", pd.products_name";
        break;
  ...
  if (PRODUCT_LIST_FILTER > 0)
  {
    if (isset($_GET['piece_style_id']))
    {
      $filterlist_sql = "select distinct c.categories_id as id, cd.categories_name as name
        ...
        where p.products_status = 1
          ...
          and pme.piece_style_id = '" . (int)$_GET['piece_style_id'] . "'
        order by cd.categories_name";
    } else {
      $filterlist_sql= "select distinct m.piece_style_id as id, m.piece_style_name as name
        from " . TABLE_PRODUCTS . " p, " .
        TABLE_PRODUCTS_TO_CATEGORIES . " p2c, " .
        TABLE_PRODUCT_PIECE_EXTRA . " pme, " .
        TABLE_PIECE_STYLE . " m
    ...
    if ($filterlist->RecordCount() > 1)
    {
      ...
      {
        $getoption_set =  true;
        $get_option_variable = 'piece_style_id';
