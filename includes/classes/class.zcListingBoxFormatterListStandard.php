  ...
  public function format(zcAbstractListingBoxBase $listingbox)
  {
    ...
    foreach ( $items as $item ) {
      ...
      if (! defined($definePrefix . 'LIST_MANUFACTURER') || (defined($definePrefix . 'LIST_MANUFACTURER') && constant($definePrefix . 'LIST_MANUFACTURER') != '0' and zen_get_show_product_switch($item ['products_id'], 'manufacturer'))) {
        $displayProductsManufacturersName = ($products_all->fields ['manufacturers_name'] != '' ? TEXT_MANUFACTURER . ' ' . $item ['manufacturers_name'] . str_repeat('<br clear="all" />', substr(constant($definePrefix . 'LIST_MANUFACTURER'), 3, 1)) : '');
      } else {
        $displayProductsManufacturersName = '';
      }

      if (! defined($definePrefix . 'LIST_PRICE') || (defined($definePrefix . 'LIST_PRICE') && (constant($definePrefix . 'LIST_PRICE') != '0' and zen_get_products_allow_add_to_cart($products_all->fields ['products_id']) == 'Y') and zen_check_show_prices() == true)) {
