<?php
/**
 * File contains just the zcTypeFilterMusicGenre class
 *
 * @package classes
 ...
 */
/**
 * class zcTypeFilterMusicGenre
 *
 * @package classes
 */
class zcTypeFilterMusicGenre extends zcAbstractTypeFilter
{
  public function handleParameterFilters()
  {
    global $current_category_id;

    $this->parts ['selectList'] [] = "m.music_genre_name as manufacturers_name";
    ...
    $this->parts ['joinTables'] ['TABLE_MUSIC_GENRE'] = array(
    ...
    if (zcRequest::hasGet('music_genre_id') && zcRequest::readGet('music_genre_id') != '') {
      $this->parts ['whereClauses'] [] = array(
          ...
          'value' => (int)zcRequest::readGet('music_genre_id'),
          'type' => 'AND'
      );
  ...
  public function buildOptionFilter()
  {
    ...
    if (PRODUCT_LIST_FILTER > 0) {
      ...
      if ($filterlist->RecordCount() > 1) {
        $this->doFilterList = true;
        if (zcRequest::hasGet('music_genre_id')) {
          $this->getOptionSet = true;
          $this->getOptionVariable = 'music_genre_id';
          $this->filterOptions = array(
              array(
                  'id' => '',
