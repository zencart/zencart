...
$define = [
    ...
    'TEXT_MAIN' => 'This script will import your product images from the filesystem to track their names in the database.<br><br>Click the "Start Scan" button below to begin the scanning process. Depending on the number of images, this may take some time. Please be patient and do not navigate away from this page until the process is complete. If the process is interrupted, simply start it again and it will resume from where it left off. <br><br><strong>Please re-run until it reports 0 images remaining to scan (in the blue status bar that will show at the top of the page).</strong><br><br>Once the scan is complete, verify that your product and category images are displaying correctly on the storefront.<br><br>You can run this tool again anytime after uploading more images via FTP and it will attempt to sync them to products based on legacy filename matching patterns.<br><br>NOTE: This script is non-destructive and does not do anything to any actual image files; it only reads their names to match up with products.<br>This script also does NOT upload any actual image files.',
