...
$columns_per_row = (int)($tplSetting->PRODUCT_LISTING_COLUMNS_PER_ROW  ?? 1);
        ...
        for ($col = 0, $n = count($column_list); $col < $n; $col++) {
            ...
            switch ($column_list[$col]) {
                ...
                case 'PRODUCT_LIST_IMAGE':
                    $lc_align = 'center';
                    $lc_text = '';
                    if (!empty($record['products_image']) || $tplSetting->PRODUCTS_IMAGE_NO_IMAGE_STATUS > 0) {
                        $lc_text .= '<div class="list-image">';
                        $lc_text .= '<a href="' . $href . '" title="' . zen_output_string_protected($listing_product_name) . '">';
                        $lc_text .= zen_image(DIR_WS_IMAGES . $record['products_image'], $listing_product_name, $tplSetting->IMAGE_PRODUCT_LISTING_WIDTH, $tplSetting->IMAGE_PRODUCT_LISTING_HEIGHT, 'loading="lazy" class="listingProductImage"');
                        $lc_text .= '</a>';
                        $lc_text .= '</div>';
                    }
                    break;
...
if ($how_many > 0 && $show_submit && $num_products_count > 0 && in_array($tplSetting->PRODUCT_LISTING_MULTIPLE_ADD_TO_CART, ['1', '3'], true)) {
    $show_top_submit_button = true;
}
if ($how_many > 0 && $show_submit && $num_products_count > 0 && $tplSetting->PRODUCT_LISTING_MULTIPLE_ADD_TO_CART >= 2) {
