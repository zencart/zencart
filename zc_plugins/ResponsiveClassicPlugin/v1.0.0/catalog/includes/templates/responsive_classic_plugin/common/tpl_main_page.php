...
<?php if (!$breadcrumb->isEmpty() && ($tplSetting->DEFINE_BREADCRUMB_STATUS == '1' || ($tplSetting->DEFINE_BREADCRUMB_STATUS == '2' && !$this_is_home_page))) { ?>
    <div id="navBreadCrumb"><?php echo $breadcrumb->trail($tplSetting->BREAD_CRUMBS_SEPARATOR); ?></div>
<?php } ?>
<!-- eof breadcrumb -->
...
if ($tplSetting->COLUMN_RIGHT_STATUS === '0' || (zen_config('CUSTOMERS_APPROVAL') === '1' && !zen_is_logged_in()) || (zen_config('CUSTOMERS_APPROVAL_AUTHORIZATION') === '1' && $tplSetting->CUSTOMERS_AUTHORIZATION_COLUMN_RIGHT_OFF === 'true' && ($_SESSION['customers_authorization'] != 0 || !zen_is_logged_in()))) {
...
<?php
  ...
  *
  */
  if (zen_config('CUSTOMERS_APPROVAL_AUTHORIZATION') === '1' && $tplSetting->CUSTOMERS_AUTHORIZATION_FOOTER_OFF === 'true' && ($_SESSION['customers_authorization'] != 0 || $_SESSION['customer_id'] == '')) {
    $flag_disable_footer = true;
  }
