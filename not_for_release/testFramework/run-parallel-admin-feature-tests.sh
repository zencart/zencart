...
run_test_file() {
    ...
    (
        ...
        if "$PHP_BIN" "$PHPUNIT_BIN" --configuration "$ROOT_DIR/phpunit.xml" --testsuite FeatureAdmin --group parallel-candidate "${EXTRA_PHPUNIT_ARGS[@]+"${EXTRA_PHPUNIT_ARGS[@]}"}" "$file" >"$output_file" 2>&1; then
            echo 0 >"$status_file"
        else
            echo $? >"$status_file"
        fi
...
for file in "${TEST_FILES[@]}"; do
    while [ "$active_jobs" -ge "$PROCESS_COUNT" ]; do
        if ! reap_one; then
            failures=$((failures + 1))
        fi
