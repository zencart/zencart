...
if ($requestFile === null || $responseFile === null) {
    fwrite(STDERR, "Usage: php execute_storefront_request.php <request-json> <response-json>\n");
    exit(1);
}
...
global $template, $current_page_base, $detect;
...
$debugPath = $payload['debug_path'] ?? null;
$capturedHeaders = [];
...
$writeResponse = static function () use ($responseFile, $request, $entrypoint, $normalizeHeaders, $extractResponseCookies, $debugPath, &$capturedHeaders): void {
    $content = '';
    if (ob_get_level() > 0) {
    ...
    if ($debugPath !== null) {
        $debugPayload = [
            ...
            'included_files_tail' => array_slice(get_included_files(), -25),
            'output' => $content,
        ];

        file_put_contents($debugPath, json_encode($debugPayload, JSON_PRETTY_PRINT));
    }
};
...
$scriptName = '/' . ltrim(basename($entrypoint), '/');
...
set_error_handler(static function (int $severity, string $message): bool {
    return str_contains($message, 'Constant IS_ADMIN_FLAG already defined');
});
