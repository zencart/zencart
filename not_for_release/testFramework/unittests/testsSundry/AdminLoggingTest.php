 ...
 * Testing Library
 */
class AdminLoggingTest extends zcTestCase
{
    ...
    public function setUp(): void
    {
        ...
        require_once(TESTCWD . 'support/zcObserverAliasTest.php');
    ...
    public function testPrepareMaliciousLogData()
    {
        ...
        $this->assertStringContainsString('[&lt;iframe]', $var[$line], 'should find converted iframe tag');
        $this->assertStringContainsString('[&lt;script]', $var[$line], 'should find the script tag converted');
    ...
    public function testUpdateNotifyAdminActivityLogEvent()
    {
        ...
        $var = file($file);
        $line = sizeof($var) - 1;
        $this->assertStringContainsString('Accessed page [testsuite]', $var[$line]); // should find the message
    }
    ...
    public function testFileLogWriterUpdateToEmptyLogFile()
    {
        ...
        $this->assertStringContainsString('Logging started.', $var[0], 'Should find an init message on first row');
        $this->assertStringNotContainsString('Logging started.', $var[1], 'Should not find an init on the next row');
        $this->assertStringNotContainsString('Log reset', $var[1], 'Should find an init on the next row');
    ...
    public function testFileLogWriterUpdateToRegularLogFile()
    {
        ...
        $this->assertStringNotContainsString('Log reset', $var[0]); // should not find a reset message
    ...
    public function testFileLogWriterReset()
    {
        ...
        $this->assertStringNotContainsString('dummy data', $var[0]); // should not find the dummy data
    ...
    {
        $zcObserverAliasTest = new zcObserverAliasTest;
        $result = $zcObserverAliasTest->fireNotifierValid();
        $this->assertTrue($result == 'foobar');
        $result = $zcObserverAliasTest->fireNotifierInvalid();
