...
//    private $VATcreated = false;
    ...
    {
        $sql = "UPDATE " . DB_PREFIX . "configuration SET configuration_value = '" . ($mode == 'on' ? '1' : '0') . "' where configuration_key = 'MODULE_SHIPPING_ITEM_TAX_CLASS'";
        $this->doDbQuery($sql);
    }
    function switchFlatShippingTax($mode = 'on')
    {
        $sql = "UPDATE " . DB_PREFIX . "configuration SET configuration_value = '" . ($mode == 'on' ? '1' : '0') . "' where configuration_key = 'MODULE_SHIPPING_FLAT_TAX_CLASS'";
        $this->doDbQuery($sql);
    }
