...
- If a plugin uses composer-managed packages, the `vendor/` folder for that plugin should be placed under `zc_plugins/<unique_key>/<version>/vendor/`. An .htaccess file should be placed in that vendor directory to block web access. The `composer.json` for that plugin can be in the plugin root directory.
