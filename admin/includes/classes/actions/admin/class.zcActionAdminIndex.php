...
class zcActionAdminIndex extends zcActionAdminBase
{
  ...
  public function doStartWizardDisplay()
  {
    ...
    $storeAddress = Request::readPost('store_address', ((STORE_NAME_ADDRESS != '') ? STORE_NAME_ADDRESS : ''));
    $storeName = Request::readPost('store_name', ((STORE_NAME != '') ? STORE_NAME : ''));
    $storeOwner = Request::readPost('store_owner', ((STORE_OWNER != '') ? STORE_OWNER : ''));
    $storeOwnerEmail = Request::readPost('store_owner_email', ((STORE_OWNER_EMAIL_ADDRESS != '') ? STORE_OWNER_EMAIL_ADDRESS : ''));
    $storeCountry = Request::readPost('store_country', ((STORE_COUNTRY != '') ? STORE_COUNTRY : ''));
    $storeZone = Request::readPost('store_zone', ((STORE_ZONE != '') ? STORE_ZONE : ''));
  ...
  {
    global $db;
    if (Request::readPost('store_name', '') != '') {
      $sql = "UPDATE " . TABLE_CONFIGURATION . " set configuration_value = :configValue: WHERE configuration_key = 'STORE_NAME'";
      $sql = $db->bindVars($sql, ':configValue:', Request::readPost('store_name'), 'string');
      $db->execute($sql);
    }
    if (Request::readPost('store_owner', '') != '') {
      $sql = "UPDATE " . TABLE_CONFIGURATION . " set configuration_value = :configValue: WHERE configuration_key = 'STORE_OWNER'";
      $sql = $db->bindVars($sql, ':configValue:', Request::readPost('store_owner'), 'string');
      $db->execute($sql);
    }
    if (Request::readPost('store_owner_email', '') != '') {
      ...
      $sql = $db->bindVars($sql, ':configValue:', Request::readPost('store_owner_email'), 'string');
      $db->execute($sql);
    }
    if (Request::readPost('store_country', '') != '') {
      ...
      $sql = $db->bindVars($sql, ':configValue:', Request::readPost('store_country'), 'integer');
      $db->execute($sql);
    }
    if (Request::readPost('store_zone', '') != '') {
      $sql = "UPDATE " . TABLE_CONFIGURATION . " set configuration_value = :configValue: WHERE configuration_key = 'STORE_ZONE'";
      $sql = $db->bindVars($sql, ':configValue:', Request::readPost('store_zone'), 'integer');
      $db->execute($sql);
    }
    if (Request::readPost('store_address', '') != '') {
      $sql = "UPDATE " . TABLE_CONFIGURATION . " set configuration_value = :configValue: WHERE configuration_key = 'STORE_NAME_ADDRESS'";
      $sql = $db->bindVars($sql, ':configValue:', Request::readPost('store_address'), 'string');
      $db->execute($sql);
    }
