...
$page = Request::readGet('cmd', basename($PHP_SELF, ".php"));
...
if (Request::readGet('cmd') != FILENAME_ALERT_PAGE) {
  if (! (Request::readGet('cmd') == FILENAME_LOGIN)) {
    if (! isset($_SESSION ['admin_id'])) {
      if (! (Request::readGet('cmd') == FILENAME_PASSWORD_FORGOTTEN)) {
        zen_redirect(zen_href_link(FILENAME_LOGIN, 'camefrom=' . Request::readGet('cmd') . '&' . zen_get_all_get_params(array(
            'cmd'
        )), 'SSL'));
      }
    }
    if (! in_array($page, array(
      ...
      if (check_page(Request::readGet('cmd'), Request::all('get')) == FALSE) {
