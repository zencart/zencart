 ...
 * @version $Id: lat9 2025 Feb 25 Modified in v2.2.0 $
 */
...
if (!$demo_check->EOF) {
    $cnt_admin = 0;
    foreach ($demo_check as $next_check) {
        $checking = $next_check['admin_pass'];
        if ($next_check['admin_name'] === 'Admin' && zen_validate_password('admin', $checking)) {
            $admin_security = true;
            $cnt_admin++;
        } elseif ($next_check['admin_name'] === 'demo' && zen_validate_password('demoonly', $checking)) {
            $admin_security = true;
            $cnt_admin++;
        }
    }
...
// Alerts for EZ-Pages
if ((int)zen_config('EZPAGES_STATUS_HEADER') === 2 && zen_is_whitelisted_admin_ip()) {
    $messageStack->add(TEXT_EZPAGES_STATUS_HEADER_ADMIN, 'caution');
}
if ((int)zen_config('EZPAGES_STATUS_FOOTER') === 2 && zen_is_whitelisted_admin_ip()) {
    $messageStack->add(TEXT_EZPAGES_STATUS_FOOTER_ADMIN, 'caution');
}
if ((int)zen_config('EZPAGES_STATUS_SIDEBOX') === 2 && zen_is_whitelisted_admin_ip()) {
...
// check activity log size for authorized admins
