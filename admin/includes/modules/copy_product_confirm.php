      ...
      if (!empty($_POST['copy_linked_categories']) && $_POST['copy_linked_categories'] == 'copy_linked_categories_yes') {
          ...
          while (!$categories_from->EOF) {//"insert ignore" as the new product already has one entry for the master category
              $db->Execute("INSERT IGNORE INTO " . TABLE_PRODUCTS_TO_CATEGORIES . "
								...
								VALUES ('" . $dup_products_id . "', '" . (int)$categories_from->fields['categories_id'] . "')");
              $messageStack->add_session(sprintf(TEXT_COPY_AS_DUPLICATE_CATEGORIES, (int)$categories_from->fields['categories_id'], $products_id, $dup_products_id),'success');
