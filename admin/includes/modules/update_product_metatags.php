 ...
 * @version $Id: update_product_metatags.php 19772 2011-10-11 15:13:26Z wilt $
 */
...
// check if new meta tags or existing
          $check_metatags_description = $db->Execute("select products_id from " . TABLE_METATAGS_PRODUCTS_DESCRIPTION . " where products_id='" . (int)$products_id . "'");
          ...
          for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
            ...
            if ($action == 'new_product_metatags') {
              ...
              if ($n == 1 && empty($_POST['metatags_title'][$language_id]) && empty($_POST['metatags_keywords'][$language_id]) && empty($_POST['metatags_description'][$language_id])) {
                $remove_products_metatag = "DELETE from " . TABLE_METATAGS_PRODUCTS_DESCRIPTION . " WHERE products_id = '" . (int)$products_id . "' and language_id = '" . (int)$language_id . "'";
