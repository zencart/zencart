...
function zen_record_admin_activity($data = '', $specific_message = '')
{
  global $db, $zco_notifier, $PHP_SELF;
  ...
  $sql_data_array = array( 'access_date' => 'now()',
          'admin_id' => (isset($_SESSION['admin_id'])) ? (int)$_SESSION['admin_id'] : 0,
          'page_accessed' =>  basename($PHP_SELF) . (!isset($_SESSION['admin_id']) || (int)$_SESSION['admin_id'] == 0 ? ' ' . (isset($data['admin_name']) ? $data['admin_name'] : (isset($data['admin_email']) ? $data['admin_email'] : '') ) : ''),
