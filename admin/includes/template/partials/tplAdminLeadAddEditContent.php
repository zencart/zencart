...
<section>
    <div class="panel">
        <?php echo zen_draw_form('AdminLeadAddEditContentForm', $_GET['cmd'],
            zen_get_all_get_params(array('action')) . 'action=' . $tplVars['pageDefinition'] ['formAction'], 'post',
            ' data-abide ' . $tplVars['pageDefinition']['enctype'] . 'class="form-horizontal"') ?>
        <fieldset>
            <legend><?php echo $tplVars['legendTitle']; ?></legend>

            <?php foreach ($tplVars['pageDefinition']['editMap'] as $field) { ?>
                <?php if (isset($tplVars['pageDefinition']['fields'][$field]['layout']['type'])) { ?>
                    <div class="form-group">
                        <div class="col-sm-9">
                            <?php if (isset($tplVars['pageDefinition']['fields'][$field]['language'])) { ?>
                                <?php require('includes/template/partials/tplLanguageTextWrapper.php'); ?>
                            <?php } else { ?>
                                <?php require('includes/template/partials/leadInputTypes/tpl' . ucfirst($tplVars['pageDefinition']['fields'][$field]['layout']['type']) . '.php'); ?>
                            <?php } ?>
                        </div>
                    </div>
                <?php } ?>
        ...
        <a href="<?php echo $tplVars['pageDefinition'] ['cancelButtonAction']; ?>">
