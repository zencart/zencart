    ...
    if ((int)$next_lang['id'] > 0 && !in_array((int)$next_lang['id'], $ary)) {
        $db->Execute(
            "INSERT INTO " . TABLE_PRODUCTS_OPTIONS_VALUES . "
             ...
             VALUES
                (" . (int)zen_config('PRODUCTS_OPTIONS_VALUES_TEXT_ID') . ", " . (int)$next_lang['id'] . ", 'TEXT')"
        );
    }
}
...
$page_param = ($currentPage !== 0) ? 'page=' . $currentPage . '&' : '';

$_GET['products_filter'] = (int)($_GET['products_filter'] ?? $products_filter ?? 0);
$products_filter = $_GET['products_filter'];

$_GET['attributes_id'] = (int)($_GET['attributes_id'] ?? 0);
        ...
        case 'set_products_filter':
            ...
            zen_redirect(
                zen_href_link(
                    ...
                    zen_get_all_get_params($exclude_array) . $page_param . 'products_filter=' . (int)$_POST['products_filter'] . '&current_category_id=' . (int)$_POST['current_category_id']
                )
            );
            break;
        ...
        case 'add_product_attributes':
            ...
            for ($i = 0; $i < count($_POST['values_id']); $i++) {
                ...
                if (!$check_duplicate->EOF) {
                    ...
                    $values_id = (int)(($products_options_type === zen_config('PRODUCTS_OPTIONS_TYPE_TEXT') || $products_options_type === zen_config('PRODUCTS_OPTIONS_TYPE_FILE')) ? zen_config('PRODUCTS_OPTIONS_VALUES_TEXT_ID') : $value_id);
                    ...
                    if (zen_config('DOWNLOAD_ENABLED') === 'true') {
                        $products_attributes_filename = zen_sanitize_download_filename($_POST['products_attributes_filename']);
                        $products_attributes_filename = zen_limit_image_filename($products_attributes_filename, TABLE_PRODUCTS_ATTRIBUTES_DOWNLOAD, 'products_attributes_filename');
            ...
            zen_redirect(zen_href_link(FILENAME_ATTRIBUTES_CONTROLLER, $page_param . 'products_filter=' . $_POST['products_id'] . '&current_category_id=' . $_POST['current_category_id']));
            break;
...
// iii 030811 added:  Enforce rule that TEXT and FILE Options use value PRODUCTS_OPTIONS_VALUES_TEXT_ID
                    ...
                    switch ($products_options_query->fields['products_options_type']) {
                        ...
                        case zen_config('PRODUCTS_OPTIONS_TYPE_FILE'):
                            $values_id = (int)zen_config('PRODUCTS_OPTIONS_VALUES_TEXT_ID');
                            break;
                        default:
...
// iii 030811 added END
            ...
            zen_redirect(zen_href_link(FILENAME_ATTRIBUTES_CONTROLLER, $page_param . 'current_category_id=' . $_POST['current_category_id'] . '&products_filter=' . $products_id));
            break;
        ...
        case 'delete_attribute':
            if (isset($_POST['delete_attribute_id'])) {
                ...
                zen_redirect(zen_href_link(FILENAME_ATTRIBUTES_CONTROLLER, $page_param . 'current_category_id=' . $_POST['current_category_id'] . '&products_filter=' . $products_filter));
            }
            break;
        ...
        case 'update_attributes_copy_to_category':
            ...
            $copy_attributes_duplicates_skipped = ($_POST['copy_attributes'] === 'copy_attributes_ignore' ? '1' : '0');
            $copy_attributes_duplicates_overwrite = ($_POST['copy_attributes'] === 'copy_attributes_update' ? '1' : '0');
            if (empty($_POST['categories_update_id'])) {
...
?>
        ...
        <div class="col-sm-4 text-right">
          <?= zen_draw_form('searchForm', FILENAME_CATEGORY_PRODUCT_LISTING, '', 'get') ?>
          <?= zen_draw_hidden_field('cmd', FILENAME_CATEGORY_PRODUCT_LISTING) ?>
          <?= (!empty($_GET['search'])) ? '<a href="' . zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING) . '" class="btn btn-default" role="button">' . IMAGE_RESET . '</a>&nbsp;&nbsp;' : '' ?>
          <?= zen_draw_label(HEADING_TITLE_SEARCH_DETAIL, 'search') . ' ' . zen_draw_input_field('search', '', 'id="search"') . zen_hide_session_id() ?>
          <?= (!empty($_GET['search'])) ? '<br>' . TEXT_INFO_SEARCH_DETAIL_FILTER . zen_db_input(zen_db_prepare_input($_GET['search'])) : '' ?>
          <?= '</form>' ?>
        </div>
      </div>
<?php
...
?>
      <div class="row">
        ...
        <div class="col-xs-6 col-sm-8">
          ...
          <?= '<a href="' . zen_href_link(FILENAME_ATTRIBUTES_CONTROLLER, 'products_filter=' . $products_filter . '&current_category_id=' . $current_category_id . $page_param) . '" class="btn btn-default" role="button">' . IMAGE_CANCEL . '</a>' ?>
        </div>
        <?= '</form>' ?>
      </div>
<?php
...
?>
      <div class="row">
        ...
        <?= zen_draw_hidden_field('products_filter', $_GET['products_filter']) ?>
        <?= zen_draw_hidden_field('current_category_id', $_GET['current_category_id']) ?>
        <?= zen_draw_hidden_field('products_options_id_all', $_GET['products_options_id_all']) ?>
...
<?php
} elseif ($action === 'attribute_features_copy_to_product') {
      ...
      <div class="row">
        ...
        <div class="form-group">
          ...
          <div class="col-sm-6 text-center">
            ...
            <?= '<a href="' . zen_href_link(FILENAME_ATTRIBUTES_CONTROLLER, $page_param . 'products_filter=' . $products_filter) . '" class="btn btn-default" role="button">' . IMAGE_CANCEL . '</a>' ?>
          </div>
        </div>
        <?= '</form>' ?>
      </div>
...
?>
      <div class="row">
        ...
        <div class="form-group">
          ...
          <div class="col-sm-6 text-center">
            <button type="submit" class="btn btn-primary"><i class="fa-solid fa-copy" aria-hidden="true"></i> <?= IMAGE_COPY ?></button>
            <?= '<a href="' . zen_href_link(FILENAME_ATTRIBUTES_CONTROLLER, $page_param . 'products_filter=' . $products_filter) . '" class="btn btn-default" role="button">' . IMAGE_CANCEL . '</a>' ?></div>
        </div>
        <?= '</form>' ?>
      </div>
<?php
...
?>
            ...
            <div class="form-group">
              <div class="col-xs-2 col-sm-1 col-md-1 col-lg-1 text-center">
                <?= ($display_priced_by_attributes ? '<span class="text-warning"><strong>' . TEXT_PRICED_BY_ATTRIBUTES . '</strong></span>' . '<br>' : '') ?>
              ...
              <div class="col-xs-10">
                <?= zen_draw_pulldown_products('products_filter', 'class="form-control" size="10" onchange="this.form.submit()"', '', true, $_GET['products_filter'], true, true, 'products_model'); ?>
...
?>
        <div class="row">
          ...
          <div class="col-sm-6"><?= $attributes_split->display_count($attributes_query_numrows, zen_config('MAX_DISPLAY_SEARCH_RESULTS'), $_GET['page'], TEXT_DISPLAY_NUMBER_OF_ATTRIBUTES) ?></div>
          <?php $exclude_array = ['page', 'attributes_id']; ?>
          <div class="col-sm-6 text-right"><?= $attributes_split->display_links($attributes_query_numrows, zen_config('MAX_DISPLAY_SEARCH_RESULTS'), zen_config('MAX_DISPLAY_PAGE_LINKS'), $_GET['page'], zen_get_all_get_params($exclude_array)) ?></div>
        </div>
        <div class="row">
          <div class="col-sm-12">
                ...
                <td colspan="10">
                  <div class="row">
                    ...
                    <div class="col-xs-6 col-sm-6 text-right">
                      ...
                      <a href="<?= zen_href_link(FILENAME_ATTRIBUTES_CONTROLLER, $page_param . 'products_filter=' . $products_filter . '&current_category_id=' . $current_category_id); ?>" class="btn btn-default" role="button"><?=IMAGE_CANCEL ?></a>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-xs-1 col-sm-1">
                      <strong><?= $attributes_value['products_attributes_id'] ?></strong>
                      <?= zen_draw_hidden_field('attribute_id', $attributes_value['products_attributes_id']) ?>
                      <?= zen_draw_hidden_field('products_id', $products_filter) ?>
                      <?= zen_draw_hidden_field('current_category_id', $current_category_id) ?>
                      <?= zen_draw_hidden_field('options_id', $attributes_value['options_id']) ?>
                    </div>
...
?>
              ...
              <tr>
                ...
                <td colspan="3" class="text-center">
                  ...
                  <a href="<?= zen_href_link(FILENAME_ATTRIBUTES_CONTROLLER, $page_param . 'products_filter=' . $products_filter . '&current_category_id=' . $current_category_id); ?>" class="btn btn-default" role="button"><?= IMAGE_CANCEL ?></a></td>
                <td colspan="3">&nbsp;</td>
              </tr>
              <tr>
                <td colspan="10"><?= zen_black_line() ?></td>
...
?>
                <td class="text-right actions">
                  ...
                  <a href="<?= zen_href_link(FILENAME_ATTRIBUTES_CONTROLLER, 'action=delete_product_attribute&attribute_id=' . $attributes_value['products_attributes_id'] . '&' . $page_param . 'products_filter=' . $products_filter . '&current_category_id=' . $current_category_id) ?>" class="btn btn-danger" role="button" data-toggle="tooltip" title="<?= TEXT_DELETE_OPTION_VALUE ?>">
                    <?= zen_icon('trash', hidden: true) ?>
                  </a>
                </td>
<?php
                ...
                <td colspan="10">
                  ...
                  <div class="row">
                    <div class="col-xs-12 col-sm-1">
                      ...
                      <?= zen_draw_hidden_field('attribute_id', $next_id) ?>
                      <?= zen_draw_hidden_field('products_id', $products_filter) ?>
                      <?= zen_draw_hidden_field('current_category_id', $current_category_id) ?>
                    </div>
<?php
