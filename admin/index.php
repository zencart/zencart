...
if (isset($_POST['action']) && $_POST['action'] == 'setup-wizard')
{
	...
	$db->execute($sql);
	$sql = "UPDATE " . TABLE_CONFIGURATION . " set configuration_value = :configValue: WHERE configuration_key = 'STORE_OWNER_EMAIL'"; 
	$sql = $db->bindVars($sql, ':configValue:', $_POST['store_owner_email'], 'string');
	$db->execute($sql);
	$sql = "UPDATE " . TABLE_CONFIGURATION . " set configuration_value = :configValue: WHERE configuration_key = 'STORE_COUNTRY'"; 
	...
	$db->execute($sql);
	$sql = "UPDATE " . TABLE_CONFIGURATION . " set configuration_value = :configValue: WHERE configuration_key = 'STORE_ADDRESS'"; 
...
//  }
  ...
  <input type="text" id="store_name" name="store_name" value="<?php echo zen_output_string_protected($storeName); ?>">
  </div>
 	</div>
