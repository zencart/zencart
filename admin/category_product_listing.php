...
$page_param = ($page_num > 1) ? "&page=$page_num" : '';
...
switch ($action) {
    case 'set_categories_sort_order':
        ...
        zen_redirect(zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, 'cPath=' . $_GET['cPath'] . $pID_param . $search_parameter . $page_param));
        break;
    ...
    case 'set_editor':
        ...
        zen_redirect(zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, 'cPath=' . $_GET['cPath'] . $pID_param . $page_param));
        break;
    case 'update_category_status':
        // disable category and products including subcategories
        if (isset($_POST['categories_id'], $_POST['categories_status'])) {
            ...
            if ($_POST['categories_status'] === '1') {//form is coming from an Enabled category which is to be changed to Disabled
                ...
                $subcategories_status = ($_POST['set_subcategories_status'] ?? '') === 'set_subcategories_status_off' ? '0' : ''; //Disable subcategories or no change?
                $products_status = ($_POST['set_products_status'] ?? '') === 'set_products_status_off' ? '0' : ''; //Disable products or no change?
            } else {//form is coming from a Disabled category which is to be changed to Enabled
                $category_status = '1'; //Enable this category
                $subcategories_status = ($_POST['set_subcategories_status'] ?? '') === 'set_subcategories_status_on' ? '1' : ''; //also Enable subcategories or no change?
                $products_status = ($_POST['set_products_status'] ?? '') === 'set_products_status_on' ? '1' : ''; //Disable products or no change?
            }

            foreach ($categories as $category) {
                $current_category = (int)$category['id'];
                //set categories_status
                if ($current_category === $categories_id) {//always update THIS category
                    zen_set_category_status($current_category, $category_status);
                } elseif ($subcategories_status !== '') {//optionally update subcategories if a change was selected
                    zen_set_category_status($current_category, $subcategories_status);
                }
                ...
                $category_products = zen_get_linked_products_for_category($current_category);
    ...
    case 'remove_type':
        if (isset($_POST['type_id'])) {
            ...
            zen_redirect(zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, 'action=edit_category&cPath=' . $_GET['cPath'] . '&cID=' . (int)$_GET['cID']));
        }
        break;
    case 'setflag':
        if (isset($_GET['flag'], $_GET['pID']) && in_array($_GET['flag'], ['0', '1'])) {
    ...
    case 'delete_category_confirm':
        ...
        if (!empty($_POST['categories_id']) && (int)$_POST['categories_id'] !== (int)TOPMOST_CATEGORY_PARENT_ID) {
            $categories_id = (int)$_POST['categories_id'];
            ...
            foreach ($categories as $category) {
                ...
                $category_products = zen_get_linked_products_for_category($category_id);

                foreach ($category_products as $category_product) {
                    $cascaded_prod_id_for_delete = $category_product;
                    $cascaded_prod_cat_for_delete = [];
                    $cascaded_prod_cat_for_delete[] = $category_id;
    ...
    case 'move_category_confirm':
        if (isset($_POST['categories_id']) && (int)$_POST['categories_id'] !== (int)$_POST['move_to_category_id']) {
            $categories_id = (int)$_POST['categories_id'];
    ...
    case 'delete_product_confirm':
        $delete_linked = (($_POST['delete_linked'] ?? '') === 'delete_linked_no') ? 'false' : 'true';
        require zen_get_admin_module_from_directory($product_type, 'delete_product_confirm.php');
        zen_redirect(zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, 'cPath=' . $cPath));
        break;
    case 'move_product_confirm':
        require zen_get_admin_module_from_directory($product_type, 'move_product_confirm.php');
        zen_redirect(zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, 'cPath=' . $new_parent_id . '&pID=' . $products_id . $page_param));
        break;
    case 'copy_product_confirm':
        ...
        if ($_POST['copy_as'] === 'duplicate' && !empty($_POST['edit_duplicate'])) {
            ...
            zen_redirect(zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, 'cPath=' . $categories_id . '&pID=' . $products_id . $page_param));
        }
        break;
    case 'delete_attributes':
        ...
        zen_redirect(zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, 'cPath=' . $cPath . '&pID=' . $_GET['products_id'] . $page_param));
        break;
    ...
    case 'update_attributes_copy_to_product':
        ...
        zen_redirect(zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, 'cPath=' . $cPath . '&pID=' . $_POST['products_id'] . $page_param));
        break;
...
<body>
    ...
    <div class="container-fluid">
        <h1>
            <a href="<?= zen_catalog_href_link('index', zen_get_path($current_category_id)) ?>" rel="noopener" target="_blank" title="<?= BOX_HEADING_CATALOG ?>">
                <?= zen_icon('popup', BOX_HEADING_CATALOG, '') ?>
            </a>
            <?= HEADING_TITLE ?>&nbsp;-&nbsp;<?= zen_output_generated_category_path($current_category_id) ?>
            <?php if ($current_category_id != 0) { ?>
            <a href="<?= zen_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&cID=' . $current_category_id . '&action=edit_category') ?>" class="" role="button">
                <?= zen_icon('edit', IMAGE_EDIT_CATEGORY, '') ?>
            </a>
            <?php } ?>
        </h1>
        <?php if ($action === '') { ?>
        <div class="row">
            ...
            <div class="col-md-4">
                ...
                <div class="form-group">
                    ...
                    echo ($page_num !== 1) ? zen_draw_hidden_field('page', $page_num) : '';
                ...
                <div class="form-group">
                    <?php if ($_SESSION['display_categories_dropdown'] === 0) { ?>
                        <div class="col-sm-6 col-md-4 control-label">
                            <a href="<?= zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, 'set_display_categories_dropdown=1' . $cID_param . '&cPath=' . $cPath . $page_param) ?>" title="<?= IMAGE_ICON_STATUS_OFF ?>">
                                <i class="fa-solid fa-xmark fa-lg txt-status-off"></i>
                            </a>
                    ...
                    <?php } else { ?>
                        <div class="col-sm-6 col-md-4 control-label">
                            <a href="<?= zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, 'set_display_categories_dropdown=0' . $cID_param . '&cPath=' . $cPath . $page_param) ?>" title="<?= IMAGE_ICON_STATUS_ON ?>">
                                <i class="fa-solid fa-check fa-lg txt-status-on"></i>
                            </a>
...
?>
                    ...
                    foreach ($categories as $category) {
                        $categories_count++;
                        $current_cID = (int)$category['categories_id'];
// Get parent_id for subcategories if search
                        if (isset($_GET['search'])) {
                            $cPath = $category['parent_id'];
                        }

                        if ((!isset($_GET['cID']) && !isset($_GET['pID']) || ((int)($_GET['cID'] ?? -1) === (int)$current_cID)) && !isset($cInfo) && !str_starts_with($action, 'new')) {
                        ...
                        ?>
                        <tr class="category-listing-row" data-cid="<?= $current_cID ?>">
                            <td class="text-right"><?= $current_cID ?></td>
                            <td class="dataTableButtonCell">
                                <a href="<?= zen_catalog_href_link('index', zen_get_path($current_cID)) ?>" rel="noopener" target="_blank" title="<?= BOX_HEADING_CATALOG ?>">
                                    <?= zen_icon('popup', BOX_HEADING_CATALOG, '') ?>
                                </a>
                                <a href="<?= zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, zen_get_path($current_cID)) ?>" class="folder">
...
?>
                            ...
                            if ($category['categories_status'] === '1') { ?>
                                <a href="<?= zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, zen_get_all_get_params(['action', 'cID', 'cPath', 'flag',]) . 'action=setflag_categories&flag=0&cID=' . $current_cID . '&cPath=' . $cPath) ?>" role="button">
                                    <?= zen_icon('enabled', IMAGE_ICON_STATUS_ON, size: 'lg', hidden: true) ?>
                                </a>
                            <?php } else { ?>
                                <a href="<?= zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, zen_get_all_get_params(['action', 'cID', 'cPath', 'flag',]) . 'action=setflag_categories&flag=1&cID=' . $current_cID . '&cPath=' . $cPath) ?>" role="button">
                            ...
                            <td class="text-right dataTableButtonCell actions">
                                <div class="btn-group">
                                    <a href="<?= zen_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&cID=' . $current_cID . '&action=edit_category' . $search_parameter) ?>" class="btn btn-sm btn-default btn-edit" data-toggle="tooltip" title="<?= ICON_EDIT ?>" role="button">
                                        <?= zen_icon('pencil', size: '', hidden: true) ?>
                                    </a>
                                    <a href="<?= zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, 'cPath=' . $cPath . '&cID=' . $current_cID . '&action=delete_category' . $page_param) ?>" class="btn btn-sm btn-default btn-delete" data-toggle="tooltip" title="<?= ICON_DELETE ?>" role="button">
                                        <?= zen_icon('trash', size: '', hidden: true) ?>
                                    </a>
                                    <a href="<?= zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, 'cPath=' . $cPath . '&cID=' . $current_cID . '&action=move_category' . $page_param) ?>" class="btn btn-sm btn-default btn-move" data-toggle="tooltip" title="<?= ICON_MOVE ?>" role="button">
                                        <strong>M</strong>
                                    </a>
                                    <?php
                                        $additional_icons = '';
                                        ...
                                        if (zen_get_category_metatag_fields($current_cID, (int)$_SESSION['languages_id'], 'metatags_keywords') || zen_get_category_metatag_fields($current_cID, (int)$_SESSION['languages_id'], 'metatags_description')) { ?>
                                        <a href="<?= zen_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&cID=' . $current_cID . '&action=edit_category_meta_tags') ?>" class="btn btn-sm btn-default btn-metatags-on" data-toggle="tooltip" title="<?= ICON_EDIT_METATAGS ?>" role="button">
                                            <?= zen_icon('metatags', size: '', hidden: true) ?>
                                        </a>
                                    <?php } else { ?>
                                        <a href="<?= zen_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&cID=' . $current_cID . '&action=edit_category_meta_tags') ?>" class="btn btn-sm btn-default btn-metatags-off" data-toggle="tooltip" title="<?= ICON_EDIT_METATAGS ?>" role="button">
                                            <?= zen_icon('metatags', size: '', hidden: true) ?>
                                        </a>
                                    <?php } ?>
<?php
...
?>
                    ...
                    $products_query_raw =
                        "SELECT DISTINCT p.products_type, p.products_id, pd.products_name, p.products_quantity,
...
// reset page when page is unknown
                    if ($page_num === 1 && !empty($_GET['pID'])) {
                        ...
                        if ($check_page->RecordCount() > $max_results) {
                            ...
                            $page_num = (int)round((($check_count / $max_results) + (fmod_round($check_count, $max_results) != 0 ? .5 : 0)));
                        }
                    }
                    $page_param  = ($page_num === 1) ? '' : "&page=$page_num";
                    $products_split = new splitPageResults($page_num, $max_results, $products_query_raw, $products_query_numrows);
...
// Get categories_id for product if search
                        ...
                        if (((!isset($_GET['pID']) && !isset($_GET['cID'])) || ((int)($_GET['pID'] ?? -1)) === (int)$product['products_id']) && !isset($pInfo) && !isset($cInfo) && !str_starts_with($action, 'new')) {
                        ...
                        <tr class="product-listing-row" data-pid="<?= $product['products_id'] ?>" data-cpath="<?= $cPath ?>" data-ptype="<?= $product['products_type'] ?>">
                            ...
                            <td class="dataTableButtonCell">
                                <a href="<?= zen_catalog_href_link($type_handler . '_info', 'cPath=' . $cPath . '&products_id=' . $product['products_id'] . '&language=' . $_SESSION['languages_code'] . '&product_type=' . $product['products_type']) ?>" rel="noopener" target="_blank">
                            ...
                            <td class="text-right dataTableButtonCell actions">
                                <div class="btn-group">
                                    ...
                                    <?php if (zen_has_product_attributes($product['products_id'], false)) { ?>
                                        <a href="<?= zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, 'cPath=' . $cPath . '&pID=' . $product['products_id'] . '&action=attribute_features' . $page_param) ?>" class="btn btn-sm btn-default btn-attributes-on" role="button" title="<?= BOX_CATALOG_CATEGORIES_ATTRIBUTES_CONTROLLER ?>"><strong>A</strong></a>
                                    <?php } else { ?>
...
// meta tags
                                    if (zen_get_product_metatag_fields($product['products_id'], (int)$_SESSION['languages_id'], 'metatags_keywords') || zen_get_product_metatag_fields($product['products_id'], (int)$_SESSION['languages_id'], 'metatags_description')) {
                                        ?>
                                        <a href="<?= zen_href_link(FILENAME_PRODUCT, 'product_type=' . $product['products_type'] . '&cPath=' . $cPath . '&pID=' . $product['products_id'] . '&action=new_product_meta_tags' . $page_param) ?>" class="btn btn-sm btn-default btn-metatags-on" role="button" title="<?= ICON_EDIT_METATAGS ?>">
                                            <i class="fa-solid fa-asterisk fa-lg" aria-hidden="true"></i>
                                        </a>
                                    <?php } else { ?>
                                        <a href="<?= zen_href_link(FILENAME_PRODUCT, 'product_type=' . $product['products_type'] . '&cPath=' . $cPath . '&pID=' . $product['products_id'] . '&action=new_product_meta_tags' . $page_param) ?>" class="btn btn-sm btn-default btn-metatags-off" role="button" title="<?= ICON_EDIT_METATAGS ?>">
                                            <i class="fa-solid fa-asterisk fa-lg" aria-hidden="true"></i>
                                        </a>
                                    <?php } ?>
                                </div>
            ...
            switch ($action) {
                case 'setflag_categories':
                    ...
                    $contents[] = [
                        'align' => 'center',
                        'text' =>
                            '<button type="submit" class="btn btn-primary me-2">' . IMAGE_UPDATE . '</button>' .
                            '<a href="' . zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, zen_get_all_get_params(['cPath', 'action',]) . 'cPath=' . $cPath) . '" class="btn btn-default" role="button">'
                                . IMAGE_CANCEL .
                            '</a>'
                    ];
                    break;
                case 'delete_category':
                    ...
                    $contents = [
                        'form' =>
                            zen_draw_form('categories', FILENAME_CATEGORY_PRODUCT_LISTING, 'action=delete_category_confirm&cPath=' . $cPath) .
                    ...
                    $contents[] = [
                        ...
                        'text' =>
                            ...
                            '<a href="' . zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, 'cPath=' . $cPath . '&cID=' . $cInfo->categories_id) . '" class="btn btn-default" role="button">' .
                                IMAGE_CANCEL .
                            '</a>'
                        ];
                    break;
                case 'move_category':
                    ...
                    $contents[] = [
                        'align' => 'center',
                        'text' =>
                            '<button type="submit" class="btn btn-primary me-2">' . IMAGE_MOVE . '</button>' .
                            '<a href="' . zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, 'cPath=' . $cPath . '&cID=' . $cInfo->categories_id) . '" class="btn btn-default" role="button">' .
                                IMAGE_CANCEL .
                            '</a>'
                    ];
                ...
                case 'attribute_features':
                    ...
                    $contents[] = [
                        'align' => 'center',
                        'text' =>
                            '<a href="' . zen_href_link(FILENAME_ATTRIBUTES_CONTROLLER, '&action=attributes_preview' . '&products_filter=' . $pInfo->products_id . '&current_category_id=' . $current_category_id) . '" class="btn btn-info me-2" role="button">' .
                                IMAGE_PREVIEW .
                            '</a>' .
                            '<a href="' . zen_href_link(FILENAME_ATTRIBUTES_CONTROLLER, 'products_filter=' . $pInfo->products_id . '&current_category_id=' . $current_category_id) . '" class="btn btn-primary" role="button">' .
                                IMAGE_EDIT_ATTRIBUTES .
                            '</a>'
                    ];
                    $contents[] = [
                        'text' =>
                            '<strong>' . TEXT_PRODUCT_ATTRIBUTES_DOWNLOADS . '</strong>' .
                            zen_has_product_attributes_downloads($pInfo->products_id) .
                    ...
                    $contents[] = [
                        'text' =>
                            TEXT_INFO_ATTRIBUTES_FEATURES_UPDATES .
                            '<strong>' . zen_get_products_name($pInfo->products_id, $_SESSION['languages_id']) . ' ID# ' . $pInfo->products_id . '</strong>' .
                            '<a href="' . zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, $cPath_pID_params . '&action=update_attributes_sort_order' . $page_param . '&products_id=' . $pInfo->products_id) . '" class="btn btn-primary btn-sm ms-2" role="button">' .
                                IMAGE_UPDATE .
                            '</a>'
                    ];
                    $contents[] = [
                        'text' =>
                            TEXT_INFO_ATTRIBUTES_FEATURES_COPY_TO_PRODUCT .
                            '<strong>' . zen_get_products_name($pInfo->products_id, $_SESSION['languages_id']) . ' ID# ' . $pInfo->products_id . '</strong>' .
                            '<a href="' . zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, $cPath_pID_params . '&action=attribute_features_copy_to_product' . $page_param . '&products_id=' . $pInfo->products_id) . '" class="btn btn-primary btn-sm ms-2" role="button">' .
                                IMAGE_COPY_TO .
                            '</a>'
                    ];
                    $contents[] = [
                        'text' =>
                            TEXT_INFO_ATTRIBUTES_FEATURES_COPY_TO_CATEGORY .
                            '<strong>' . zen_get_products_name($pInfo->products_id, $_SESSION['languages_id']) . ' ID# ' . $pInfo->products_id . '</strong>' .
                            '<a href="' . zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, $cPath_pID_params . '&action=attribute_features_copy_to_category' . $page_param . '&products_id=' . $pInfo->products_id) . '" class="btn btn-primary btn-sm ms-2" role="button">' .
                                IMAGE_COPY_TO .
                            '</a>'
                    ];
                    $contents[] = [
                        'align' => 'center',
                        'text' =>
                            '<a href="' . zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, $cPath_pID_params . $page_param) . '" class="btn btn-default" role="button">' .
                                IMAGE_CANCEL .
                            '</a>'
                    ];
                    break;
                ...
                case 'attribute_features_copy_to_product':
                    ...
                    $contents = [
                        'form' =>
                            zen_draw_form('products', FILENAME_CATEGORY_PRODUCT_LISTING, 'action=update_attributes_copy_to_product&cPath=' . $cPath . $page_param, 'post', 'class="form-horizontal"') .
                            zen_draw_hidden_field('products_id', $pInfo->products_id) .
                            zen_draw_hidden_field('products_update_id', $_GET['products_update_id']) .
                            zen_draw_hidden_field('copy_attributes', $_GET['copy_attributes'] ?? '')
                    ];
                    $contents[] = [
                        'text' =>
                            zen_draw_label(TEXT_COPY_ATTRIBUTES_CONDITIONS, 'copy_attributes', 'class="control-label"') .
                            '<div class="radio"><label>' .
                                zen_draw_radio_field('copy_attributes', 'copy_attributes_delete', true) .
                                TEXT_COPY_ATTRIBUTES_DELETE .
                            '</label></div>' .
                            '<div class="radio"><label>' .
                                zen_draw_radio_field('copy_attributes', 'copy_attributes_update') .
                                TEXT_COPY_ATTRIBUTES_UPDATE .
                            '</label></div>' .
                            '<div class="radio"><label>' .
                                zen_draw_radio_field('copy_attributes', 'copy_attributes_ignore') .
                                TEXT_COPY_ATTRIBUTES_IGNORE .
                            '</label></div>'
                    ];
                    ...
                    $contents[] = [
                        ...
                        'text' =>
                            '<button type="submit" class="btn btn-primary">' .
                                IMAGE_COPY_TO .
                            '</button>' .
                            '<a href="' . zen_href_link(FILENAME_CATEGORY_PRODUCT_LISTING, 'cPath=' . $cPath . '&pID=' . $pInfo->products_id . $page_param) . '" class="btn btn-default" role="button">' .
                                IMAGE_CANCEL .
                            '</a>'
                    ];
                    break;
                ...
                case 'attribute_features_copy_to_category':
                    ...
                    $contents = [
                        'form' =>
                            zen_draw_form('products', FILENAME_CATEGORY_PRODUCT_LISTING, 'action=update_attributes_copy_to_category&cPath=' . $cPath . $page_param, 'post', 'class="form-horizontal"') .
                            zen_draw_hidden_field('products_id', $pInfo->products_id) .
                            zen_draw_hidden_field('categories_update_id', $_GET['categories_update_id']) .
                            zen_draw_hidden_field('copy_attributes', $_GET['copy_attributes'] ?? '')
                    ];
                    $contents[] = [
                        'text' =>
        ...
        if ($action === '') {
            ...
            <?php if ($products_query_numrows > 0) { ?>
            <div class="row">
