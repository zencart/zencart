...
if (isset($_GET['action']) && $_GET['action']) {
  switch ($_GET['action']) {
    case 'save':
      ...
      zen_redirect(zen_href_link(FILENAME_LAYOUT_CONTROLLER, (isset($_GET['page']) ? 'page=' . $_GET['page'] : '') . '&cID=' . $box_id));
      break;
    case 'deleteconfirm':
      ...
      zen_redirect(zen_href_link(FILENAME_LAYOUT_CONTROLLER, (isset($_GET['page']) ? 'page=' . $_GET['page'] : '')));
      break;
    case 'reset_defaults':
      if ($_POST['action'] == 'reset_defaults') {
        ...
        foreach ($reset_boxes as $reset_box) {
          $db->Execute("UPDATE " . TABLE_LAYOUT_BOXES . "
                        SET layout_box_status = " . zen_db_input($reset_box['layout_box_status']) . ",
                            layout_box_location = " . zen_db_input($reset_box['layout_box_location']) . ",
                            layout_box_sort_order = " . zen_db_input($reset_box['layout_box_sort_order']) . ",
                            layout_box_sort_order_single = " . zen_db_input($reset_box['layout_box_sort_order_single']) . ",
                            layout_box_status_single = " . zen_db_input($reset_box['layout_box_status_single']) . "
...
//  if ( (is_object($bInfo)) && ($column_controller->fields['layout_id'] == $bInfo->layout_id) ) {
                if (isset($bInfo) && is_object($bInfo) && ($column_controller->fields['layout_id'] == $bInfo->layout_id)) {
                  echo '              <tr class="dataTableRowSelected" onclick="document.location.href=\'' . zen_href_link(FILENAME_LAYOUT_CONTROLLER, (isset($_GET['page']) ? 'page=' . $_GET['page'] : '') . '&cID=' . $bInfo->layout_id . '&action=edit') . '\'" role="button">' . "\n";
                } else {
                  echo '              <tr class="dataTableRow" onclick="document.location.href=\'' . zen_href_link(FILENAME_LAYOUT_CONTROLLER, (isset($_GET['page']) ? 'page=' . $_GET['page'] : '') . '&cID=' . $column_controller->fields['layout_id']) . '\'" role="button">' . "\n";
                }
                ?>
              ...
              <td class="dataTableContent text-right"><?php echo ( (file_exists($boxes_directory . $column_controller->fields['layout_box_name']) || file_exists($boxes_directory_template . $column_controller->fields['layout_box_name'])) ? TEXT_GOOD_BOX : TEXT_BAD_BOX); ?><?php
                  if (isset($bInfo) && is_object($bInfo) && ($column_controller->fields['layout_id'] == $bInfo->layout_id)) {
                    ...
                    echo '<a href="' . zen_href_link(FILENAME_LAYOUT_CONTROLLER, (isset($_GET['page']) ? 'page=' . $_GET['page'] : '') . '&cID=' . $column_controller->fields['layout_id']) . '">' . zen_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>';
                  }
                  ?>&nbsp;</td>
              </tr>
        ...
        <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3 configurationColumnRight">
            ...
            switch (isset($_GET['action']) && $_GET['action']) {
              case 'edit':
                ...
                $contents = array('form' => zen_draw_form('column_controller', FILENAME_LAYOUT_CONTROLLER, (isset($_GET['page']) ? 'page=' . $_GET['page'] : '') . '&cID=' . $bInfo->layout_id . '&action=save' . '&layout_box_name=' . $bInfo->layout_box_name, 'post', 'class="form-horizontal"'));
                ...
                $contents[] = array('align' => 'text-center', 'text' => '<br><button type="submit" class="btn btn-primary">' . IMAGE_UPDATE . '</button> <a href="' . zen_href_link(FILENAME_LAYOUT_CONTROLLER, (isset($_GET['page']) ? 'page=' . $_GET['page'] : '') . '&cID=' . $bInfo->layout_id . '&layout_box_name=' . $bInfo->layout_box_name) . '" class="btn btn-default" role="button">' . IMAGE_CANCEL . '</a>');
                break;
              case 'delete':
                ...
                $contents[] = array('align' => 'text-center', 'text' => '<br><button type="submit" class="btn btn-danger">' . IMAGE_UPDATE . '</button> <a href="' . zen_href_link(FILENAME_LAYOUT_CONTROLLER, (isset($_GET['page']) ? 'page=' . $_GET['page'] : '') . '&cID=' . $bInfo->layout_id) . '" class="btn btn-default" role="button">' . IMAGE_CANCEL . '</a>');
                break;
              default:
                if (is_object($bInfo)) {
                  $heading[] = array('text' => '<h4>' . TEXT_INFO_LAYOUT_BOX . $bInfo->layout_box_name . '</h4>');
                  $contents[] = array('text' => '<a href="' . zen_href_link(FILENAME_LAYOUT_CONTROLLER, (isset($_GET['page']) ? 'page=' . $_GET['page'] : '') . '&cID=' . $bInfo->layout_id . '&action=edit') . '" class="btn btn-primary" role="button">' . IMAGE_EDIT . '</a>');
                  ...
                  if (!(file_exists($boxes_directory . $bInfo->layout_box_name) || file_exists($boxes_directory_template . $bInfo->layout_box_name))) {
                    ...
                    $contents[] = array('text' => '<a href="' . zen_href_link(FILENAME_LAYOUT_CONTROLLER, (isset($_GET['page']) ? 'page=' . $_GET['page'] : '') . '&cID=' . $bInfo->layout_id . '&action=delete' . '&layout_box_name=' . $bInfo->layout_box_name) . '" class="btn btn-warning" role="button">' . IMAGE_DELETE . '</a>');
                  }
                }
                break;
            }
