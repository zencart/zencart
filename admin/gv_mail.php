        ...
        case ('send_email_to_user'):
            ...
            foreach ($mail as $row) {
                ...
                $insert_query = $db->Execute(
                    "INSERT INTO " . TABLE_COUPONS . " (coupon_code, coupon_type, coupon_amount, date_created)
                ...
                $db->Execute(
                    "INSERT INTO " . TABLE_COUPON_EMAIL_TRACK . " (coupon_id, customer_id_sent, sent_firstname, emailed_to, date_sent)
...
//  if (form.elements[field_cust] && form.elements[field_cust].type != "hidden" && form.elements[field_input] && form.elements[field_input].type != "hidden") {
      ...
      function check_form(form_name) {
        ...
        check_recipient('customers_email_address', 'email_to', "<?= ERROR_NO_CUSTOMER_SELECTED ?>");
...
?>
  ...
  switch ($action) {
      case 'preview' :
          ...
          ?>
          <?= zen_draw_form('mail', FILENAME_GV_MAIL, 'action=send_email_to_user') ?>
          <table class="table">
            <tr>
              <td class="text-right col-sm-3"><b><?= TEXT_FROM ?></b></td>
              <td><?= htmlspecialchars(stripslashes($_POST['from']), ENT_COMPAT, CHARSET, true) ?></td>
            </tr>
            <tr>
              <td class="text-right"><b><?= TEXT_TO ?></b></td>
              <td><?= $mail_sent_to . (!empty($_POST['email_to']) && $mail_sent_to_name != '' ? ' - ' . $mail_sent_to_name : '') ?></td>
            </tr>
            <tr>
          ...
          <div class="form-group">
            <div class="col-sm-offset-3 col-sm-9">
              <?= ($_POST['amount'] <= 0 ? '' : '<button type="submit" class="btn btn-primary" name="send">' . IMAGE_SEND . '</button>&nbsp;') ?><button type="submit" class="btn btn-info" name="back"><?= IMAGE_BACK ?></button>&nbsp;<button type="submit" class="btn btn-default" name="cancel"><?= IMAGE_CANCEL ?></button>
            </div>
          </div>
          <?php
      ...
      default:
          ...
          <div class="row">
            <div class="col-sm-offset-8 col-sm-4 text-right">
              <?= zen_draw_form('set_editor_form', FILENAME_GV_MAIL, '', 'get', 'class="form-horizontal"') ?>
              <div class="form-group">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-9 col-md-6">
              <?= zen_draw_input_field('from', htmlspecialchars(zen_config('EMAIL_FROM', ''), ENT_COMPAT, CHARSET, true), 'size="50" class="form-control" id="from"') ?>
            </div>
          </div>
          <hr>
          <div class="form-group">
            <?= zen_draw_label(TEXT_TO_CUSTOMERS, 'customers_email_address', 'class="control-label col-sm-3"') ?>
            <div class="col-sm-9 col-md-6">
              <?= zen_draw_pull_down_menu('customers_email_address', $customers, (!empty($_POST['customers_email_address']) ? $_POST['customers_email_address'] : ''), 'class="form-control" id="customers_email_address"') ?>
            </div>
          </div>
          <div class="form-group">
            <div class="col-sm-12"><?= TEXT_TO_EMAIL_INFO ?></div>
          </div>
          <div class="form-group">
            <?= zen_draw_label(TEXT_TO_EMAIL, 'email_to', 'class="control-label col-sm-3"') ?>
            <div class="col-sm-9 col-md-6">
              <?= zen_draw_input_field('email_to', (!empty($_POST['email_to']) ? $_POST['email_to'] : ''), 'size="25" class="form-control" id="email_to"', false, 'email') ?>
            </div>
          </div>
          <div class="form-group">
            <?= zen_draw_label(TEXT_TO_EMAIL_NAME, 'email_to_name', 'class="control-label col-sm-3"') ?>
            <div class="col-sm-9 col-md-6">
              <?= zen_draw_input_field('email_to_name', (!empty($_POST['email_to_name']) ? $_POST['email_to_name'] : ''), 'size="25" class="form-control" id="email_to_name"', false) ?>
            </div>
          </div>
          <hr>
          <div class="form-group">
            ...
            <div class="col-sm-9 col-md-6">
              <?= zen_draw_input_field('subject', (!empty($_POST['subject']) ? $_POST['subject'] : ''), 'size="50" class="form-control" id="subject"', true) ?>
            </div>
          </div>
          <div class="form-group">
            <?= zen_draw_label(TEXT_AMOUNT, 'amount', 'class="control-label col-sm-3"') ?>
            <div class="col-sm-9 col-md-6">
              <?= zen_draw_input_field('amount', (!empty($_POST['amount']) ? $_POST['amount'] : ''), 'step="any" class="form-control" id="amount"', true, 'number') ?>
              <span class="help-block"><?= TEXT_AMOUNT_INFO ?></span>
            </div>
          </div>
          <hr>
          <div class="form-group">
            <div class="col-sm-12"><?= TEXT_MESSAGE_INFO ?></div>
          </div>
          <?php if (zen_config('EMAIL_USE_HTML') === 'true') { ?>
            <div class="form-group">
              <?= zen_draw_label(TEXT_HTML_MESSAGE, 'message_html', 'class="control-label col-sm-3"') ?>
              <div class="col-sm-9 col-md-6">
                <?= zen_draw_textarea_field('message_html', 'soft', '', '10', htmlspecialchars(empty($_POST['message_html']) ? '' : stripslashes($_POST['message_html']), ENT_COMPAT, CHARSET, true), 'id="message_html" class="editorHook form-control"') ?>
              </div>
            </div>
          <?php } ?>
          <div class="form-group">
            ...
            <div class="col-sm-9 col-md-6">
              <?= zen_draw_textarea_field('message', 'soft', '', '10', htmlspecialchars(empty($_POST['message']) ? '' : stripslashes($_POST['message']), ENT_COMPAT, CHARSET, true), 'id="message" class="noEditor tt form-control"') ?>
            </div>
          </div>
          <div class="form-group">
