    ...
    if (($action == 'new') || ($action == 'edit')) {
      ...
      <script>
        function session_win() {
            window.open("<?= zen_href_link(FILENAME_SALEMAKER_INFO) ?>", "salemaker_info", "height=460,width=600,scrollbars=yes,resizable=yes").focus();
        }
...
//            window.open("<?= zen_href_link(FILENAME_SALEMAKER_POPUP, 'cid=' . $category['categories_id']) ?>//", "salemaker_info", "height=460,width=600,scrollbars=yes,resizable=yes").focus();
//        }
*/?>
...
<?php } ?>
  ...
  <div class="container-fluid">
    ...
    if (($action == 'new') || ($action == 'edit')) {
      ...
      <div class="row">
        ...
        <div class="col-sm-6 text-right">
          <button type="submit" class="btn btn-primary"><?= ($form_action == 'insert' ? IMAGE_INSERT : IMAGE_UPDATE) ?></button>
            <a href="<?= zen_href_link(FILENAME_SALEMAKER, 'page=' . $_GET['page'] . (!empty($_GET['sID']) ? '&sID=' . $_GET['sID'] : '')) ?>" class="btn btn-default" role="button"><?= IMAGE_CANCEL ?></a>
        </div>
      </div>
      ...
      <div class="form-group">
          <?= zen_draw_label(TEXT_SALEMAKER_DEDUCTION, 'deduction', 'class="col-sm-3 control-label"') ?>
        <div class="col-sm-9 col-md-6">
          <div class="col-sm-5"><?= zen_draw_input_field('deduction', $sInfo->sale_deduction_value, 'size="8" class="form-control"') ?></div>
          <div class="col-sm-2"><?= TEXT_SALEMAKER_DEDUCTION_TYPE ?></div>
          <div class="col-sm-5"><?= zen_draw_pull_down_menu('type', $deduction_type_array, $sInfo->sale_deduction_type, 'class="form-control"') ?></div>
        </div>
      </div>
      <div class="form-group">
          <?= zen_draw_label(TEXT_SALEMAKER_PRICERANGE_FROM, 'from', 'class="col-sm-3 control-label"') ?>
        <div class="col-sm-9 col-md-6">
          <div class="col-sm-5"><?= zen_draw_input_field('from', $sInfo->sale_pricerange_from, 'size="8" class="form-control"') ?></div>
          <div class="col-sm-2"><?= TEXT_SALEMAKER_PRICERANGE_TO ?></div>
          <div class="col-sm-5"><?= zen_draw_input_field('to', $sInfo->sale_pricerange_to, 'size="8" class="form-control"') ?></div>
        </div>
      </div>
      <div class="form-group">
          <?= zen_draw_label(TEXT_SALEMAKER_SPECIALS_CONDITION, 'condition', 'class="col-sm-3 control-label"') ?>
        <div class="col-sm-9 col-md-6">
            <?= zen_draw_pull_down_menu('condition', $specials_condition_array, $sInfo->sale_specials_condition, 'class="form-control"') ?>
        </div>
      </div>
      <div class="form-group">
          <?= zen_draw_label(TEXT_SALEMAKER_DATE_START, 'start', 'class="col-sm-3 control-label"') ?>
        <div class="col-sm-9 col-md-6">
          <script>
            StartDate.writeControl();
            StartDate.dateFormat = "<?= DATE_FORMAT_SPIFFYCAL ?>";
          </script>
        </div>
      </div>
      <div class="form-group">
          <?= zen_draw_label(TEXT_SALEMAKER_DATE_END, 'end', 'class="col-sm-3 control-label"') ?>
        <div class="col-sm-9 col-md-6">
          <script>
            EndDate.writeControl();
            EndDate.dateFormat = "<?= DATE_FORMAT_SPIFFYCAL ?>";
          </script>
        </div>
      </div>
      <?php
...
// set Entire Catalog when set
      ...
      <div class="form-group" onClick="RowClick('0')">
        <div class="col-sm-offset-3">
          <div>
            <label><?= zen_draw_checkbox_field('categories[]', '0', $zc_check_all_cats) ?>&nbsp;<?= TEXT_SALEMAKER_TOP ?></label>
          </div>
        </div>
      </div>
      <div class="form-group">
        <div class="col-sm-offset-3">
          <?= zen_icon('caret-right', size: 'lg') ?>&nbsp;<?= TEXT_SALEMAKER_CATEGORIES ?>
        </div>
      </div>
      <?php
          ...
          <table class="table table-striped table-hover">
            <thead>
              <tr class="dataTableHeadingRow">
                <th class="dataTableHeadingContent"><?= TABLE_HEADING_SALE_NAME ?></th>
                <th class="dataTableHeadingContent right"><?= TABLE_HEADING_SALE_DEDUCTION ?></th>
                <th class="dataTableHeadingContent"></th>
                <th class="dataTableHeadingContent center"><?= TABLE_HEADING_SALE_DATE_START ?></th>
                <th class="dataTableHeadingContent center"><?= TABLE_HEADING_SALE_DATE_END ?></th>
                <th class="dataTableHeadingContent center"><?= TABLE_HEADING_STATUS ?></th>
            ...
            <tbody>
                ...
                foreach ($salemaker_sales as $salemaker_sale) {
                  ...
                  <tr class="dataTableRow" onclick="document.location.href = '<?= zen_href_link(FILENAME_SALEMAKER, 'page=' . $_GET['page'] . '&sID=' . $salemaker_sale['sale_id']) ?>'">
                    <?php } ?>
                  <td  class="dataTableContent text-left"><?= $salemaker_sale['sale_name'] ?></td>
                  <td  class="dataTableContent text-right"><?= $salemaker_sale['sale_deduction_value'] ?></td>
                  <td  class="dataTableContent text-left"><?= $deduction_type_array[$salemaker_sale['sale_deduction_type']]['text'] ?></td>
                  <td  class="dataTableContent text-center"><?= (($salemaker_sale['sale_date_start'] == '0001-01-01') ? TEXT_SALEMAKER_IMMEDIATELY : zen_date_short($salemaker_sale['sale_date_start'])) ?></td>
                  <td  class="dataTableContent text-center"><?= (($salemaker_sale['sale_date_end'] == '0001-01-01') ? TEXT_SALEMAKER_NEVER : zen_date_short($salemaker_sale['sale_date_end'])) ?></td>
        ...
        <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3 configurationColumnRight">
            ...
            switch ($action) {
              ...
              case 'delete':
                ...
                $contents[] = ['text' => '<br><b>' . $sInfo->sale_name . '</b>'];
