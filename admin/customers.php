        ...
        switch ($action) {
            ...
            case 'update':
                ...
                if ($error == false) {
                    ...
                    if (ACCOUNT_DOB == 'true') {
                        $sql_data_array[] = ['fieldName' => 'customers_dob', 'value' => ($customers_dob == '0001-01-01 00:00:00' ? '0001-01-01 00:00:00' : zen_date_raw($customers_dob)), 'type' => 'date'];
                    }
                    ...
                    $zco_notifier->notify('NOTIFY_ADMIN_CUSTOMERS_B4_ADDRESS_UPDATE', ['customers_id' => $customers_id, 'address_book_id' => $default_address_id], $sql_data_array);
                    ...
                    zen_redirect(zen_href_link(FILENAME_CUSTOMERS, zen_get_all_get_params(['cID', 'action']) . 'cID=' . $customers_id, 'NONSSL'));
                } elseif ($error == true) {
    ...
    <div class="container-fluid">
        ...
        <?php
            if ($action == 'edit' || $action == 'update') {
                ...
                <?php
                echo zen_draw_form('customers', FILENAME_CUSTOMERS, zen_get_all_get_params(['action']) . 'action=update', 'post', 'onsubmit="return check_form(customers);" class="form-horizontal"', true);
                            ...
                            <div class="row">
                                <div class="col-sm-4">
                                    ...
                                    <?php foreach (zen_get_all_customer_groups() as $group) { ?>
                                        <div class="checkbox">
                                            <label>
                                                <input type="checkbox" name="customer_groups[]" value="<?php echo $group['id']; ?>" <?php if (array_key_exists($group['id'], $groups_already_in)) {
                                                    echo 'checked';
                                                } ?>>
                                                <?php echo $group['text']; ?>
                                            </label>
                ...
                <div class="row text-right">
                    ...
                    <a href="<?php echo zen_href_link(FILENAME_CUSTOMERS, zen_get_all_get_params(['action'])); ?>" class="btn btn-default"><?php echo IMAGE_CANCEL; ?></a>
                </div>
                <?php echo '</form>'; ?>
                <?php
                ...
                <div class="row">
                    <fieldset>
                        ...
                        <div class="buttonRow forward"><a
                                    href="<?php echo zen_href_link(FILENAME_CUSTOMERS, 'action=list_addresses_done' . '&cID=' . $_GET['cID'] . ($_GET['page'] > 0 ? '&page=' . $_GET['page'] : ''), 'NONSSL'); ?>"
                                    class="btn btn-default" role="button"><?php echo IMAGE_BACK; ?></a></div>
                    </fieldset>
                </div>
                <?php
            } else { ?>
                        ...
                        <table class="table table-hover">
                            ...
                            <tr class="dataTableHeadingRow">
                                ...
                                <th class="dataTableHeadingContent">
                                    ...
                                    <br>
                                    <a href="<?php echo zen_href_link(basename($PHP_SELF), zen_get_all_get_params(['list_order', 'page']) . 'list_order=lastname', 'NONSSL'); ?>"><?php echo($_GET['list_order'] == 'lastname' ? '<span class="SortOrderHeader">' . TEXT_ASC . '</span>' : '<span class="SortOrderHeaderLink">' . TEXT_ASC . '</span>'); ?></a>&nbsp;
                                    <a href="<?php echo zen_href_link(basename($PHP_SELF), zen_get_all_get_params(['list_order', 'page']) . 'list_order=lastname-desc', 'NONSSL'); ?>"><?php echo($_GET['list_order'] == 'lastname-desc' ? '<span class="SortOrderHeader">' . TEXT_DESC . '</span>' : '<span class="SortOrderHeaderLink">' . TEXT_DESC . '</span>'); ?></a>
                                </th>
                                <th class="dataTableHeadingContent">
                                    ...
                                    <br>
                                    <a href="<?php echo zen_href_link(basename($PHP_SELF), zen_get_all_get_params(['list_order', 'page']) . 'list_order=firstname', 'NONSSL'); ?>"><?php echo($_GET['list_order'] == 'firstname' ? '<span class="SortOrderHeader">' . TEXT_ASC . '</span>' : '<span class="SortOrderHeaderLink">' . TEXT_ASC . '</span>'); ?></a>&nbsp;
                                    <a href="<?php echo zen_href_link(basename($PHP_SELF), zen_get_all_get_params(['list_order', 'page']) . 'list_order=firstname-desc', 'NONSSL'); ?>"><?php echo($_GET['list_order'] == 'firstname-desc' ? '<span class="SortOrderHeader">' . TEXT_DESC . '</span>' : '<span class="SortOrderHeaderLink">' . TEXT_DESC . '</span>'); ?></a>
                                </th>
                                <?php if (ACCOUNT_COMPANY === 'true') { ?>
                                    <th class="dataTableHeadingContent">
                                        ...
                                        <br>
                                        <a href="<?php echo zen_href_link(basename($PHP_SELF), zen_get_all_get_params(['list_order', 'page']) . 'list_order=company', 'NONSSL'); ?>"><?php echo($_GET['list_order'] == 'company' ? '<span class="SortOrderHeader">' . TEXT_ASC . '</span>' : '<span class="SortOrderHeaderLink">' . TEXT_ASC . '</span>'); ?></a>&nbsp;
                                        <a href="<?php echo zen_href_link(basename($PHP_SELF), zen_get_all_get_params(['list_order', 'page']) . 'list_order=company-desc', 'NONSSL'); ?>"><?php echo($_GET['list_order'] == 'company-desc' ? '<span class="SortOrderHeader">' . TEXT_DESC . '</span>' : '<span class="SortOrderHeaderLink">' . TEXT_DESC . '</span>'); ?></a>
                                    </th>
                                <?php }
                                ...
                                <th class="dataTableHeadingContent">
                                    ...
                                    <br>
                                    <a href="<?php echo zen_href_link(basename($PHP_SELF), zen_get_all_get_params(['list_order', 'page']) . 'list_order=id-asc', 'NONSSL'); ?>"><?php echo($_GET['list_order'] == 'id-asc' ? '<span class="SortOrderHeader">' . TEXT_ASC . '</span>' : '<span class="SortOrderHeaderLink">' . TEXT_ASC . '</span>'); ?></a>&nbsp;
                                    <a href="<?php echo zen_href_link(basename($PHP_SELF), zen_get_all_get_params(['list_order', 'page']) . 'list_order=id-desc', 'NONSSL'); ?>"><?php echo($_GET['list_order'] == 'id-desc' ? '<span class="SortOrderHeader">' . TEXT_DESC . '</span>' : '<span class="SortOrderHeaderLink">' . TEXT_DESC . '</span>'); ?></a>
                                </th>
                                ...
                                <th class="dataTableHeadingContent">
                                    ...
                                    <br>
                                    <a href="<?php echo zen_href_link(basename($PHP_SELF), zen_get_all_get_params(['list_order', 'page']) . 'list_order=login-asc', 'NONSSL'); ?>"><?php echo($_GET['list_order'] == 'login-asc' ? '<span class="SortOrderHeader">' . TEXT_ASC . '</span>' : '<span class="SortOrderHeaderLink">' . TEXT_ASC . '</span>'); ?></a>&nbsp;
                                    <a href="<?php echo zen_href_link(basename($PHP_SELF), zen_get_all_get_params(['list_order', 'page']) . 'list_order=login-desc', 'NONSSL'); ?>"><?php echo($_GET['list_order'] == 'login-desc' ? '<span class="SortOrderHeader">' . TEXT_DESC . '</span>' : '<span class="SortOrderHeaderLink">' . TEXT_DESC . '</span>'); ?></a>
                                </th>
                                ...
                                <th class="dataTableHeadingContent">
                                    ...
                                    <br>
                                    <a href="<?php echo zen_href_link(basename($PHP_SELF), zen_get_all_get_params(['list_order', 'page']) . 'list_order=group-asc', 'NONSSL'); ?>"><?php echo($_GET['list_order'] == 'group-asc' ? '<span class="SortOrderHeader">' . TEXT_ASC . '</span>' : '<span class="SortOrderHeaderLink">' . TEXT_ASC . '</span>'); ?></a>&nbsp;
                                    <a href="<?php echo zen_href_link(basename($PHP_SELF), zen_get_all_get_params(['list_order', 'page']) . 'list_order=group-desc', 'NONSSL'); ?>"><?php echo($_GET['list_order'] == 'group-desc' ? '<span class="SortOrderHeader">' . TEXT_DESC . '</span>' : '<span class="SortOrderHeaderLink">' . TEXT_DESC . '</span>'); ?></a>
                                </th>

                                <?php if (defined('MODULE_ORDER_TOTAL_GV_STATUS') && MODULE_ORDER_TOTAL_GV_STATUS == 'true') { ?>
                                    <th class="dataTableHeadingContent text-right">
                                        <?php echo(($_GET['list_order'] == 'gv_balance-asc' or $_GET['list_order'] == 'gv_balance-desc') ? '<span class="SortOrderHeader">' . TABLE_HEADING_GV_AMOUNT . '</span>' : TABLE_HEADING_GV_AMOUNT); ?>
                                        <br>
                                        <a href="<?php echo zen_href_link(basename($PHP_SELF), zen_get_all_get_params(['list_order', 'page']) . 'list_order=gv_balance-asc', 'NONSSL'); ?>"><?php echo($_GET['list_order'] == 'gv_balance-asc' ? '<span class="SortOrderHeader">' . TEXT_ASC . '</span>' : '<span class="SortOrderHeaderLink">' . TEXT_ASC . '</span>'); ?></a>&nbsp;
                                        <a href="<?php echo zen_href_link(basename($PHP_SELF), zen_get_all_get_params(['list_order', 'page']) . 'list_order=gv_balance-desc', 'NONSSL'); ?>"><?php echo($_GET['list_order'] == 'gv_balance-desc' ? '<span class="SortOrderHeader">' . TEXT_DESC . '</span>' : '<span class="SortOrderHeaderLink">' . TEXT_DESC . '</span>'); ?></a>
                                    </th>
                                <?php } ?>
                                ...
                                <th class="dataTableHeadingContent text-center">
                                    ...
                                    <br>
                                    <a href="<?php echo zen_href_link(basename($PHP_SELF), zen_get_all_get_params(['list_order', 'page']) . 'list_order=approval-asc', 'NONSSL'); ?>"><?php echo($_GET['list_order'] == 'approval-asc' ? '<span class="SortOrderHeader">' . TEXT_ASC . '</span>' : '<span class="SortOrderHeaderLink">' . TEXT_ASC . '</span>'); ?></a>&nbsp;
                                    <a href="<?php echo zen_href_link(basename($PHP_SELF), zen_get_all_get_params(['list_order', 'page']) . 'list_order=approval-desc', 'NONSSL'); ?>"><?php echo($_GET['list_order'] == 'approval-desc' ? '<span class="SortOrderHeader">' . TEXT_DESC . '</span>' : '<span class="SortOrderHeaderLink">' . TEXT_DESC . '</span>'); ?></a>
                                </th>
                            ...
                            <?php
                                ...
                                $zco_notifier->notify('NOTIFY_ADMIN_CUSTOMERS_LISTING_NEW_FIELDS', [], $new_fields, $disp_order);
...
//    zen_redirect(zen_href_link(FILENAME_CUSTOMERS, 'cID=' . $_GET['cID'] . (isset($_GET['page']) ? '&page=' . $_GET['page'] : ''), 'NONSSL'));
                                ...
                                foreach ($customers as $result) {
                                    ...
                                    <td class="dataTableContent text-right">
                                        ...
                                        <?php } else { ?>
                                            <a href="<?php echo zen_href_link(FILENAME_CUSTOMERS, zen_get_all_get_params(['cID']) . 'cID=' . $customer['customers_id'] . ($_GET['page'] > 0 ? '&page=' . $_GET['page'] : ''), 'NONSSL'); ?>"
                        ...
                        <?php
                            ...
                            switch ($action) {
                                case 'confirm':
                                    ...
                                    $contents = ['form' => zen_draw_form('customers', FILENAME_CUSTOMERS, zen_get_all_get_params(['cID', 'action', 'search']) . 'action=deleteconfirm', 'post', '', true) . zen_draw_hidden_field('cID', $cInfo->customers_id)];
                                    ...
                                    if (isset($cInfo->number_of_reviews) && ($cInfo->number_of_reviews) > 0) {
                                        $contents[] = ['text' => '<br>' . zen_draw_checkbox_field('delete_reviews', 'on', true) . ' ' . sprintf(TEXT_DELETE_REVIEWS, $cInfo->number_of_reviews)];
                                    }
                                    $contents[] = [
                                        'align' => 'text-center',
                                        'text' => '<br>
                                               ...
                                               <a href="' . zen_href_link(FILENAME_CUSTOMERS, zen_get_all_get_params(['cID', 'action']) . 'cID=' . $cInfo->customers_id, 'NONSSL') . '" class="btn btn-default" role="button">' . IMAGE_CANCEL . '</a>',
                                    ];
                                    break;
                                case 'pwreset':
                                    $heading[] = ['text' => '<h4>' . TEXT_INFO_HEADING_RESET_CUSTOMER_PASSWORD . '</h4>'];
                                    $contents = ['form' => zen_draw_form('customers', FILENAME_CUSTOMERS, zen_get_all_get_params(['cID', 'action', 'search']) . 'action=pwdresetconfirm', 'post', 'id="pReset" class="form-horizontal"', true) . zen_draw_hidden_field('cID', $cInfo->customers_id)];
                                    $contents[] = ['text' => TEXT_PWDRESET_INTRO . '<br><br><strong>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</strong>'];
                                    $contents[] = ['text' => '<br>' . zen_draw_label(TEXT_CUST_NEW_PASSWORD, 'newpassword', 'class="control-label"') . zen_draw_input_field('newpassword', '', 'maxlength="40" autofocus="autofocus" autocomplete="off" id="newpassword" class="form-control"', false, 'text', false)];
                                    $contents[] = ['text' => '<br>' . zen_draw_label(TEXT_CUST_CONFIRM_PASSWORD, 'newpasswordConfirm', 'class="control-label"') . zen_draw_input_field('newpasswordConfirm', '', 'maxlength="40" autocomplete="off" id="newpasswordConfirm" class="form-control"', false, 'text', false)];
                                    $contents[] = ['align' => 'text-center', 'text' => '<br><button type="submit" class="btn btn-warning">' . IMAGE_RESET_PWD . '</button> <a href="' . zen_href_link(FILENAME_CUSTOMERS, zen_get_all_get_params(['cID', 'action']) . 'cID=' . $cInfo->customers_id) . '" class="btn btn-default" role="button">' . IMAGE_CANCEL . '</a>'];
                                    break;
                                default:
                                    ...
                                    if (isset($cInfo) && is_object($cInfo)) {
                                        $heading[] = ['text' => '<h4>' . TABLE_HEADING_ID . $cInfo->customers_id . ' ' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</h4>' . '<br>' . $cInfo->customers_email_address];

                                        $contents[] = ['align' => 'text-center', 'text' => '<a href="' . zen_href_link(FILENAME_CUSTOMERS, zen_get_all_get_params(['cID', 'action', 'search']) . 'cID=' . $cInfo->customers_id . '&action=edit', 'NONSSL') . '" class="btn btn-primary" role="button">' . IMAGE_EDIT . '</a> <a href="' . zen_href_link(FILENAME_CUSTOMERS, zen_get_all_get_params(['cID', 'action', 'search']) . 'cID=' . $cInfo->customers_id . '&action=confirm', 'NONSSL') . '" class="btn btn-warning" role="button">' . IMAGE_DELETE . '</a>'];
                                        $contents[] = ['align' => 'text-center', 'text' => ($cInfo->number_of_orders > 0 ? '<a href="' . zen_href_link(FILENAME_ORDERS, 'cID=' . $cInfo->customers_id, 'NONSSL') . '" class="btn btn-default" role="button">' . IMAGE_ORDERS . '</a>' : '') . ' <a href="' . zen_href_link(FILENAME_MAIL, 'origin=customers.php&customer=' . $cInfo->customers_email_address . '&cID=' . $cInfo->customers_id, 'NONSSL') . '" class="btn btn-default" role="button">' . IMAGE_EMAIL . '</a>'];
                                        $contents[] = ['align' => 'text-center', 'text' => '<a href="' . zen_href_link(FILENAME_CUSTOMERS, zen_get_all_get_params(['cID', 'action', 'search']) . 'cID=' . $cInfo->customers_id . '&action=pwreset') . '" class="btn btn-warning" role="button">' . IMAGE_RESET_PWD . '</a>'];
                                        ...
                                        if ($place_order_override === false && zen_admin_authorized_to_place_order()) {
                                            ...
                                            if (defined('EMP_LOGIN_AUTOMATIC') && EMP_LOGIN_AUTOMATIC == 'true' && ENABLE_SSL_CATALOG == 'true') {
                                                ...
                                                $timestamp = time();
                                                $hmacpostdata = [
                                                    'cid' => $cInfo->customers_id, 'aid' => $_SESSION['admin_id'],
                                        ...
                                        $contents[] = ['text' => '<br>' . TEXT_DATE_ACCOUNT_CREATED . ' ' . zen_date_short($cInfo->date_account_created)];
                                        if (!empty($cInfo->registration_ip)) {
                                            ...
                                            $contents[] = ['text' => '<br>' . TEXT_REGISTRATION_IP . ' ' . $lookup_link . $cInfo->registration_ip . '</a>'];
                                        }
                                        $contents[] = ['text' => '<br>' . TEXT_DATE_ACCOUNT_LAST_MODIFIED . ' ' . zen_date_short($cInfo->date_account_last_modified)];
                                        $contents[] = ['text' => '<br>' . TEXT_INFO_DATE_LAST_LOGON . ' ' . zen_date_short($cInfo->date_of_last_login)];
                                        if (!empty($cInfo->last_login_ip)) {
                                            $contents[] = ['text' => '<br>' . TEXT_LAST_LOGIN_IP . ' ' . $cInfo->last_login_ip];
                                        }
                                        $contents[] = ['text' => '<br>' . TEXT_INFO_NUMBER_OF_LOGONS . ' ' . $cInfo->number_of_logins];

                                        $contents[] = ['text' => '<br>' . TEXT_INFO_GV_AMOUNT . ' ' . $currencies->format($cInfo->gv_balance)];

                                        $contents[] = ['text' => '<br>' . TEXT_INFO_NUMBER_OF_ORDERS . ' ' . $cInfo->number_of_orders];

                                        if (!empty($cInfo->lifetime_value)) {
                                            $contents[] = ['text' => TEXT_INFO_LIFETIME_VALUE . ' ' . $currencies->format($cInfo->lifetime_value)];
                                            $contents[] = ['text' => TEXT_INFO_LAST_ORDER . ' ' . zen_date_short($cInfo->last_order['date_purchased']) . '<br>' . TEXT_INFO_ORDERS_TOTAL . ' ' . $cInfo->last_order['order_total']];
                                        }
                                        $contents[] = ['text' => '<br>' . TEXT_INFO_COUNTRY . ' ' . $cInfo->country_iso];
                                        $contents[] = ['text' => '<br>' . TEXT_INFO_NUMBER_OF_REVIEWS . ' ' . $cInfo->number_of_reviews];
                                        $contents[] = ['text' => '<br>' . CUSTOMERS_REFERRAL . ' ' . $cInfo->customers_referral];
                                    }
                                    break;
                            }
                ...
                <div class="row">
                    <table class="table">
                        <tr>
                            ...
                            <td class="text-right"><?php echo $customers_split->display_links($customers_query_numrows, MAX_DISPLAY_SEARCH_RESULTS_CUSTOMER, MAX_DISPLAY_PAGE_LINKS, $_GET['page'], zen_get_all_get_params(['page', 'info', 'x', 'y', 'cID'])); ?></td>
                        </tr>
