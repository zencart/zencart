...
if ($_GET['action']=='add_product' && $_POST['products_drop']) {
    $test_query = $db->Execute(
        "SELECT * 
           FROM " . TABLE_COUPON_RESTRICT . "
          WHERE coupon_id = '" . $_GET['cid'] . "' 
            AND product_id = '" . (int)$_POST['products_drop'] . "'"
    );
    if ($test_query->RecordCount() < 1) {
        $status = 'N';
        if ($_POST['restrict_status']=='Deny') $status = 'Y';
        ...
        if ($_POST['products_drop'] < 0) {
            ...
            if ($_GET['build_cat'] > 0 && $_POST['products_drop'] == -1) {
                ...
                $new_products_query = 
                    "SELECT products_id 
                      ...
                      WHERE categories_id = '" . $_GET['build_cat'] . "' 
            ...
            if ($_GET['build_man'] > 0 && $_POST['products_drop'] == -1) {
                ...
                $new_products_query = 
                    "SELECT products_id 
                       FROM " . TABLE_PRODUCTS . " 
                      WHERE manufacturers_id = '" . $_GET['build_man'] . "' 
                        AND products_id NOT IN (
                                SELECT product_id 
                                  FROM " . TABLE_COUPON_RESTRICT . " 
                                 WHERE coupon_id = '" . $_GET['cid'] . "'
                ...
                if ($_POST['products_drop'] == -1) {
                    $db->Execute(
                        "INSERT INTO " . TABLE_COUPON_RESTRICT . "
                         ...
                         VALUES 
                            ('" . $_GET['cid'] . "', '" . $new_products->fields['products_id'] . "', '" . $status . "')"
                    );
                } else {
...
$products = $db->Execute(
    "SELECT p.products_id, pd.products_name 
      ...
      WHERE p.products_id = pd.products_id 
        ...
        AND p2c.categories_id = '" . $_POST['cPath_prod'] . "'
    ...
    $products = $db->Execute(
        "SELECT p.products_id, pd.products_name 
          ...
          WHERE p.products_id = pd.products_id 
            ...
            AND m.manufacturers_id = '" . (int)$current_manufacturers_id . "'
          ORDER BY pd.products_name");
    $products_array = array();
}
