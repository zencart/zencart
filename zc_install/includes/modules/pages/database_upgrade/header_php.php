...
//that way only the "earliest-required" upgrade is suggested first.
    $needs_v1_6_0=false;
    if (!$dbinfo->version160) {
      $sniffer_text =  ' upgrade v1.5.1 to v1.6.0';
      $needs_v1_6_0=true;
    }
    $needs_v1_5_1=false;
...
//          if (!$dbinfo->version138 || $dbinfo->version139) continue;  // if prerequisite not completed, or already done, skip
       ...
       case '1.5.0':  // upgrading from v1.5.0 TO 1.5.1
            ...
            break;
       case '1.5.1':  // upgrading from v1.5.1 TO 1.6.0
            $sniffer_file = '_upgrade_zencart_151_to_160.sql';
            if (ZC_UPG_DEBUG2==true) echo $sniffer_file.'<br>';
            $got_v1_6_0 = true; //after processing this step, this will be the new version-level
            $db_upgraded_to_version='1.6.0';
            break;
          default:
