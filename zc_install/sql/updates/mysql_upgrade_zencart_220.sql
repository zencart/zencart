...
UPDATE configuration SET configuration_description = 'CSS Buttons<br>Use CSS buttons instead of images (GIF/JPG)?<br>Button styles must be configured in the stylesheet if you enable this option.<br>Yes - Use css buttons<br>No - Use images buttons<br>If - Use images if exist, if not use css buttons', set_function = 'zen_cfg_select_option(array(\'No\', \'Yes\', \'If\'), ' WHERE configuration_key = 'IMAGE_USE_CSS_BUTTONS';
